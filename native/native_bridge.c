...
static void log_jvmti_failure(const wchar_t *operation, jvmtiError error,
    ...
    umbra_log(L"%ls failed: jvmti=%d (%hs), target=%hs", operation,
...
static int contains_umbra_callback(const unsigned char *bytes, jint length) {
    return contains_bytes(bytes, length, "gg/umbra/")
            || contains_bytes(bytes, length, "gg.umbra.");
}
...
static void update_persisted_class(JNIEnv *env, jclass target,
    ...
    unsigned char *copy = NULL;
    int should_persist = contains_umbra_callback(bytes, length);

    if (should_persist) {
        copy = (unsigned char *)HeapAlloc(
                GetProcessHeap(), 0, (SIZE_T)length);
        if (copy == NULL) {
            umbra_log(L"unable to retain transformed class bytecode");
            return;
        }
        memcpy(copy, bytes, (size_t)length);
    }
    ...
    if (entry == NULL || entry->target == NULL) {
        ...
        ReleaseSRWLockExclusive(&g_persisted_class_lock);
        umbra_log(L"unable to retain transformed class identity");
        return;
    }
    entry->bytes = copy;
    entry->length = length;
...
jint umbra_initialize_jvmti(JavaVM *vm) {
    ...
    if (get_env_result != JNI_OK || g_jvmti == NULL) {
        umbra_log(L"JVMTI 1.2 is unavailable: %d", get_env_result);
        g_jvmti = NULL;
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        umbra_log(L"GetPotentialCapabilities failed: %d", error);
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        umbra_log(L"AddCapabilities failed: %d", error);
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        umbra_log(L"SetEventCallbacks failed: %d", error);
        return JNI_ERR;
    }
    error = (*g_jvmti)->SetEventNotificationMode(g_jvmti, JVMTI_ENABLE,
            JVMTI_EVENT_CLASS_FILE_LOAD_HOOK, NULL);
    if (error != JVMTI_ERROR_NONE) {
        umbra_log(L"Enable ClassFileLoadHook failed: %d", error);
        return JNI_ERR;
    }
...
static void JNICALL native_cpy(
    ...
    if (memory == NULL) {
        umbra_log(L"cpy GlobalAlloc failed: %lu", GetLastError());
        goto cleanup;
    }
    destination = GlobalLock(memory);
    if (destination == NULL) {
        umbra_log(L"cpy GlobalLock failed: %lu", GetLastError());
        goto cleanup;
    }
    ...
    if (!clipboard_open) {
        umbra_log(L"cpy OpenClipboard failed: %lu", GetLastError());
        goto cleanup;
    }
    if (!EmptyClipboard()) {
        umbra_log(L"cpy EmptyClipboard failed: %lu", GetLastError());
        goto cleanup;
    }
    if (SetClipboardData(CF_TEXT, memory) == NULL) {
        umbra_log(L"cpy SetClipboardData failed: %lu", GetLastError());
        goto cleanup;
    }
    memory = NULL;
...
static jbyteArray JNICALL native_gcb(JNIEnv *env, jclass bridge, jclass target) {
    ...
    if (error == JVMTI_ERROR_NONE && g_capture_bytes != NULL
        ...
        umbra_log(L"gcb captured byte count: %d", g_capture_length);
    }
...
static LRESULT CALLBACK lwjgl3_window_proc(
    ...
    if (env != NULL && g_bridge_class != NULL && g_bridge_om != NULL
        ...
        if ((*env)->ExceptionCheck(env)) {
            umbra_log_pending_exception(env, L"LWJGL3 window input callback");
            handled = JNI_FALSE;
        }
    }
    if (attached) {
...
static void register_lwjgl3_window(JNIEnv *env) {
    ...
    if (window == NULL) {
        umbra_log(L"trs step 23: LWJGL3 window was not found");
        return;
    }
    ...
    if (original == NULL && GetLastError() != ERROR_SUCCESS) {
        umbra_log(L"failed to subclass LWJGL3 window: %lu", GetLastError());
        return;
    }
    ...
    if (g_bridge_class != NULL && g_bridge_wh != NULL) {
        ...
        if ((*env)->ExceptionCheck(env)) {
            umbra_log_pending_exception(env, L"initialize LWJGL3 window handle");
        }
    }
    umbra_log(L"subclassed LWJGL3 window for input notifications");
}
...
static void JNICALL native_trs(JNIEnv *env, jclass bridge, jint step) {
    ...
    (void)bridge;
    umbra_loader_report_progress((int)step);
    ...
    if (error != JVMTI_ERROR_NONE || classes == NULL) {
        umbra_log(L"trs GetLoadedClasses failed: %d", error);
        return;
    }
    for (index = 0; index < class_count; ++index) {
        char *signature = NULL;
    ...
    if (windows_display != NULL) {
        ...
        if ((*env)->RegisterNatives(env, windows_display, &update_method, 1) == JNI_OK) {
            ...
            umbra_log(L"registered org.lwjgl.opengl.WindowsDisplay.nUpdate");
        } else {
            umbra_log_pending_exception(env, L"RegisterNatives WindowsDisplay.nUpdate");
        }
    } else {
        umbra_log(L"trs step 23: WindowsDisplay is not loaded; trying LWJGL3 window");
 ...
 * raise UnsatisfiedLinkError. sce is reachable from the payload (Umbra.logError).
...
static void JNICALL native_sce(JNIEnv *env, jclass bridge, jstring message) {
    ...
    (void)bridge;
    if (message == NULL) {
        umbra_log(L"client error report: <null>");
        return;
    }
    chars = (*env)->GetStringUTFChars(env, message, NULL);
    if (chars != NULL) {
        umbra_log(L"client error report: %hs", chars);
...
static jstring JNICALL bridge_gat(JNIEnv *env, jclass bridge) {
    ...
    token = umbra_loader_access_token();
...
jint umbra_register_native_bridge(JNIEnv *env, jclass bridge_class) {
    ...
    if (result != JNI_OK) {
        umbra_log_pending_exception(env, L"RegisterNatives NativeBridge");
        return result;
    }
    ...
    if (g_bridge_class == NULL || g_bridge_om == NULL || g_bridge_wh == NULL) {
        umbra_log_pending_exception(env, L"resolve NativeBridge input callbacks");
        return JNI_ERR;
    }
    umbra_log(L"registered NativeBridge methods (9 sample + gat + cpy + 5 stub)");
    return JNI_OK;
}

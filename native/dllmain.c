...
#define UMBRA_PRODUCT_JAR_RESOURCE_ID 421
...
void umbra_log(const wchar_t *format, ...) {
    ...
    _snwprintf_s(log_path, sizeof(log_path) / sizeof(log_path[0]), _TRUNCATE,
            L"%ls\\umbra-native.log", directory);
...
void umbra_log_pending_exception(JNIEnv *env, const wchar_t *context) {
    ...
    if (env == NULL || !(*env)->ExceptionCheck(env)) {
        umbra_log(L"%ls failed without a Java exception", context);
        return;
    }
    ...
    if (text == NULL || (*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        umbra_log(L"%ls raised an unreadable Java exception", context);
        return;
    }
    ...
    if (characters != NULL) {
        ...
        umbra_log(L"%ls: %ls", context, buffer);
    }
}
...
static int materialize_embedded_product_jar(
    ...
    resource = FindResourceW(g_module,
            MAKEINTRESOURCEW(UMBRA_PRODUCT_JAR_RESOURCE_ID),
            MAKEINTRESOURCEW(10));
    if (resource == NULL) {
        umbra_log(L"embedded product JAR resource %d is missing",
                UMBRA_PRODUCT_JAR_RESOURCE_ID);
        return 0;
    }
    ...
    if (bytes == NULL || size < 4 || bytes[0] != 'P' || bytes[1] != 'K') {
        umbra_log(L"embedded product JAR resource is invalid");
        return 0;
    }
    if (GetTempPathW((DWORD)(sizeof(temp_root) / sizeof(temp_root[0])),
            temp_root) == 0) {
        umbra_log(L"GetTempPathW failed: %lu", GetLastError());
        return 0;
    }
    _snwprintf_s(temp_directory,
            sizeof(temp_directory) / sizeof(temp_directory[0]), _TRUNCATE,
            L"%lsUmbraCache", temp_root);
    if (!CreateDirectoryW(temp_directory, NULL)
            && GetLastError() != ERROR_ALREADY_EXISTS) {
        umbra_log(L"CreateDirectoryW failed: %lu", GetLastError());
        return 0;
    }
    if (_snwprintf_s(jar_path, jar_capacity, _TRUNCATE,
            L"%ls\\umbra-product-%lu.jar", temp_directory,
            GetCurrentProcessId()) < 0) {
        umbra_log(L"temporary product JAR path is too long");
        return 0;
    }
    ...
    if (file == INVALID_HANDLE_VALUE) {
        umbra_log(L"CreateFileW for embedded product JAR failed: %lu",
                GetLastError());
        return 0;
    }
    while (offset < size) {
        ...
        if (!WriteFile(file, bytes + offset, remaining, &written, NULL)
                || written == 0) {
            umbra_log(L"WriteFile for embedded product JAR failed: %lu",
                    GetLastError());
            goto cleanup;
        }
        offset += written;
    ...
    if (!FlushFileBuffers(file)) {
        umbra_log(L"FlushFileBuffers for embedded product JAR failed: %lu",
                GetLastError());
        goto cleanup;
    }
    result = 1;
...
cleanup:
    ...
    if (!result) {
        ...
        umbra_log(L"materialized embedded product JAR: %ls (%lu bytes)",
                jar_path, size);
    }
    return result;
}
...
static jobject find_client_class_loader(JNIEnv *env) {
    ...
    if (error != JVMTI_ERROR_NONE || threads == NULL) {
        umbra_log(L"GetAllThreads failed: %d", error);
        return NULL;
    }
 ...
 * same gg.umbra class identities without replacing Forge's global fallback.
 *
...
static int install_modular_payload_loader(
    ...
    if (runtime_loader_class == NULL) {
        umbra_log_pending_exception(env, L"resolve runtime ClassLoader type");
        return -1;
    }
    ...
    if (package_routes_field == NULL) {
        ...
        if (modular_loader_marker != NULL) {
            umbra_log(L"ModLauncher package routing field is unavailable");
            return -1;
        }
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionClear(env);
    ...
    if (loader_class == NULL || url_loader_init == NULL
        ...
        umbra_log_pending_exception(env,
    ...
    payload_loader_name = (*env)->NewStringUTF(env,
            "gg.umbra.runtime.ForgePayloadClassLoader");
    ...
    if (build_package_routes == NULL || payload_loader == NULL
        ...
        umbra_log_pending_exception(env, L"create modular payload ClassLoader");
        return -1;
    }
    ...
    if (package_routes == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"build ModLauncher payload package routes");
        return -1;
    }
    (*env)->SetObjectField(env, *loader, package_routes_field, package_routes);
    if ((*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"install ModLauncher payload package routes");
        return -1;
    }

    /* Fail before any JVMTI redefine if the exact crashing reference is hidden. */
    target_event_name = (*env)->NewStringUTF(env,
            "gg.umbra.event.impl.EventRenderWorldPassExecutorDrain");
    ...
    *loader = payload_loader;
    umbra_log(L"installed payload package routes on ModLauncher ModuleClassLoader");
    return 1;
}
...
static int install_system_payload_search(
    ...
    if (g_jvmti == NULL) {
        umbra_log(L"JVMTI is unavailable for system ClassLoader search");
        return -1;
    }
    loader_class = (*env)->FindClass(env, "java/lang/ClassLoader");
    ...
    if (system_loader == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"ClassLoader.getSystemClassLoader");
        return -1;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        umbra_log(L"AddToSystemClassLoaderSearch failed: %d", error);
        return -1;
    }
    ...
    umbra_log(L"appended product JAR to system ClassLoader search");
    return 1;
}
...
static int install_fabric_payload_search(
    ...
    if (launcher == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"FabricLauncherBase.getLauncher");
        return -1;
    }
    ...
    if (get_target_loader == NULL || add_to_class_path == NULL) {
        umbra_log_pending_exception(env, L"resolve FabricLauncher methods");
        return -1;
    }
    target_loader = (*env)->CallObjectMethod(
            env, launcher, get_target_loader);
    if (target_loader == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"FabricLauncher.getTargetClassLoader");
        return -1;
    }
    ...
    if (path == NULL || allowed_prefixes == NULL
            || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"create Fabric payload path");
        return -1;
    }
    (*env)->CallVoidMethod(env, launcher, add_to_class_path,
            path, allowed_prefixes);
    if ((*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"FabricLauncher.addToClassPath");
        return -1;
    }
    ...
    if (payload_event_loader == NULL || (*env)->ExceptionCheck(env)
        ...
        if ((*env)->ExceptionCheck(env)) {
            ...
            umbra_log(L"Fabric payload event used the wrong ClassLoader");
        }
        return -1;
    }
    ...
    *loader = retained_target_loader;
    umbra_log(L"appended product JAR to Fabric Knot ClassLoader");
    return 1;
}
...
static int add_jar_to_loader(
    ...
    if (url_loader_class == NULL || url_class == NULL) {
        umbra_log_pending_exception(env, L"resolve URLClassLoader classes");
        return 0;
    }
    ...
    if (file_init == NULL || to_uri == NULL || to_url == NULL) {
        umbra_log_pending_exception(env, L"resolve product JAR URL methods");
        return 0;
    }
    path = new_wide_string(env, jar_path);
    ...
    if (url == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"create product JAR URL");
        return 0;
    }
    ...
    if ((*env)->IsInstanceOf(env, *loader, url_loader_class)) {
        ...
        if (add_url == NULL) {
            umbra_log_pending_exception(env, L"resolve URLClassLoader.addURL");
            return 0;
        }
        (*env)->CallVoidMethod(env, *loader, add_url, url);
        if ((*env)->ExceptionCheck(env)) {
            umbra_log_pending_exception(env, L"URLClassLoader.addURL");
            return 0;
        }
        return 1;
    }
    ...
    if (delegated_loader_field != NULL) {
        ...
        if (delegated_loader != NULL
            ...
            umbra_log(L"appended product JAR to ModLauncher delegated ClassLoader");
            return 1;
        }
    ...
    if (url_loader_init == NULL || urls == NULL) {
        umbra_log_pending_exception(env, L"resolve child URLClassLoader constructor");
        return 0;
    }
    (*env)->SetObjectArrayElement(env, urls, 0, url);
    ...
    if (child_loader == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"create child product URLClassLoader");
        return 0;
    }
    (*env)->DeleteLocalRef(env, *loader);
    *loader = child_loader;
    umbra_log(L"created child URLClassLoader for non-URL Minecraft ClassLoader");
    return 1;
}
...
static jclass load_bridge_class(JNIEnv *env, jobject loader) {
    ...
    jmethodID load_class = loader_class == NULL ? NULL : (*env)->GetMethodID(
            env, loader_class, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
    jstring name = (*env)->NewStringUTF(env, "gg.umbra.runtime.NativeBridge");
    jclass result;
    ...
    if (result == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"load gg.umbra.runtime.NativeBridge");
        return NULL;
    }
    return result;
}
...
static int set_current_context_class_loader(JNIEnv *env, jobject loader) {
    ...
    if (thread_class == NULL) {
        umbra_log_pending_exception(env, L"resolve java.lang.Thread");
        return 0;
    }
    ...
    if (current_thread == NULL || set_context_loader == NULL) {
        umbra_log_pending_exception(env, L"resolve Thread context ClassLoader methods");
        return 0;
    }
    thread = (*env)->CallStaticObjectMethod(env, thread_class, current_thread);
    if (thread == NULL || (*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"Thread.currentThread");
        return 0;
    }
    (*env)->CallVoidMethod(env, thread, set_context_loader, loader);
    if ((*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"Thread.setContextClassLoader");
        return 0;
    }
    return 1;
}
...
static int pin_native_module(void) {
    ...
    if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS
        ...
        umbra_log(L"GetModuleHandleExW(PIN) failed: %lu", GetLastError());
        return 0;
    }
    return 1;
}
...
static int call_bridge_start(JNIEnv *env, jclass bridge_class) {
    ...
    if (start == NULL) {
        umbra_log_pending_exception(env, L"resolve NativeBridge.start");
        return 0;
    }
    (*env)->CallStaticVoidMethod(env, bridge_class, start);
    if ((*env)->ExceptionCheck(env)) {
        umbra_log_pending_exception(env, L"NativeBridge.start");
        return 0;
    }
    return 1;
}
...
static DWORD WINAPI bootstrap_thread(LPVOID parameter) {
    ...
    if (!umbra_loader_bootstrap_initialize()) {
        umbra_log(L"Loader token bootstrap is invalid");
        exit_code = 6;
        goto cleanup;
    }
    Sleep(150);
    ...
    if (vm == NULL || vm_count < 1) {
        umbra_log(L"JNI_GetCreatedJavaVMs returned no VM");
        exit_code = 4;
        goto cleanup;
    }
    if ((*vm)->AttachCurrentThreadAsDaemon(vm, (void **)&env, NULL) != JNI_OK
            || env == NULL) {
        umbra_log(L"AttachCurrentThreadAsDaemon failed");
        exit_code = 5;
        goto cleanup;
    }
    attached = 1;
    if (umbra_initialize_jvmti(vm) != JNI_OK) {
        goto cleanup;
    }
    ...
    if (loader == NULL) {
        umbra_log(L"Minecraft client/render thread was not found within 60 seconds");
        goto cleanup;
    }
    ...
    if (umbra_register_native_bridge(env, bridge_class) != JNI_OK) {
        goto cleanup;
    }
    registered = 1;
    ...
    umbra_log(L"NativeBridge linked from %ls", jar_path);
    if (!call_bridge_start(env, bridge_class)) {
        goto cleanup;
    }
    umbra_loader_report_completed();
    umbra_log(L"NativeBridge.start completed; injection is active");
    completed = 1;
    exit_code = 0;
...
cleanup:
    if (!completed) {
        ...
        umbra_loader_report_failure(failure);
    }
    if (attached) {
        (*vm)->DetachCurrentThread(vm);
    }
    if (!registered && worker_module != NULL) {
        umbra_log(L"bootstrap failed before native registration; unloading DLL");
        FreeLibraryAndExitThread(worker_module, exit_code);
    }
    if (!completed) {
        umbra_log(L"bootstrap did not complete; DLL remains loaded because natives may be registered");
    }
    return exit_code;
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    bridge_class = (*env)->FindClass(env, "gg/umbra/runtime/NativeBridge");
    if (bridge_class == NULL
            || umbra_register_native_bridge(env, bridge_class) != JNI_OK) {
        umbra_log_pending_exception(env, L"JNI_OnLoad NativeBridge registration");
        return JNI_ERR;
    }
    umbra_log(L"JNI_OnLoad completed");
...
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM *vm, void *reserved) {
    ...
    (void)reserved;
    if ((*vm)->GetEnv(vm, (void **)&env, JNI_VERSION_1_8) == JNI_OK) {
        umbra_release_native_bridge(env);
    }
    umbra_loader_bootstrap_clear();
}

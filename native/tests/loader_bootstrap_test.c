...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && strcmp(argv[1], "standalone") == 0) {
        if (!umbra_loader_bootstrap_initialize()
                || strcmp(umbra_loader_access_token(), "0") != 0) {
    ...
    _snwprintf_s(mapping_name, sizeof(mapping_name) / sizeof(mapping_name[0]),
            _TRUNCATE, L"Local\\Umbra.Bootstrap.%lu", process_id);
    _snwprintf_s(ack_name, sizeof(ack_name) / sizeof(ack_name[0]),
            _TRUNCATE, L"Local\\Umbra.BootstrapAck.%lu", process_id);
    ...
    block = mapping == NULL ? NULL : (UmbraBootstrapV2 *)MapViewOfFile(
    ...
    block->magic = invalid ? 0 : UMBRA_BOOTSTRAP_MAGIC;
    ...
    if (invalid) {
        if (umbra_loader_bootstrap_initialize()
                ...
                || block->status != UMBRA_BOOTSTRAP_STATUS_FAILED) {
        ...
        if (!umbra_loader_bootstrap_initialize()
                || WaitForSingleObject(ack, 1000) != WAIT_OBJECT_0
                || block->status != UMBRA_BOOTSTRAP_STATUS_CONSUMED
                || strcmp(umbra_loader_access_token(), "persistent-test-token") != 0) {
            fprintf(stderr, "socket token bootstrap failed\n");
            goto cleanup;
        }
        umbra_loader_report_progress(23);

...
static int default_dll_path(wchar_t *output, DWORD capacity) {
    ...
    if ((size_t)(file_name - output) + wcslen(L"UmbraNative.dll") + 1
            > capacity) {
        return 0;
    }
    wcscpy(file_name, L"UmbraNative.dll");
    attributes = GetFileAttributesW(output);
    return attributes != INVALID_FILE_ATTRIBUTES
            && (attributes & FILE_ATTRIBUTE_DIRECTORY) == 0;
}
...
static size_t enumerate_candidates(
    ...
    for (read_index = 0; read_index < write_index; ++read_index) {
        HANDLE process = OpenProcess(
                PROCESS_QUERY_INFORMATION | PROCESS_VM_READ,
                FALSE, candidates[read_index].process_id);
        if (process != NULL) {
            PROCESS_MEMORY_COUNTERS counters;
            if (GetProcessMemoryInfo(process, &counters, sizeof(counters))) {
                candidates[read_index].working_set = counters.WorkingSetSize;
            }
            CloseHandle(process);
        }
    }
...
static void render_selector(const process_candidate *candidates, size_t count,
    ...
    wprintf(L"Umbra Injector\n");
    wprintf(L"DLL: %ls\n\n", dll_path);
    wprintf(L"Select your Minecraft version (Up/Down, Enter to inject, Esc to quit)\n\n");
    if (count == 0) {
...
static int inject_library(DWORD process_id, const wchar_t *dll_path) {
    ...
    if (result == 0) {
        fwprintf(stderr, L"LoadLibraryW returned, but the DLL is not mapped. "
                L"Inspect umbra-native.log for bootstrap failure.\n");
        goto cleanup;
    }
...
static void usage(const wchar_t *program) {
    fwprintf(stderr,
            L"Usage: %ls [UmbraNative.dll]\n"
            L"       %ls <minecraft-pid> <UmbraNative.dll>\n"
            L"Without arguments you will be prompted to select a Java game window.\n"
            L"Specify a PID and DLL path to inject directly.\n"
            L"The injected DLL loads and starts the Java product automatically.\n",
            program, program);
}

static int inject_and_report(DWORD process_id, const wchar_t *dll_path) {
    int injection_result = inject_library(process_id, dll_path);
...
int wmain(int argc, wchar_t **argv) {
    ...
    if (pid_arg != NULL && dll_arg != NULL) {
        process_id = wcstoul(pid_arg, &end, 10);
        if (process_id == 0 || end == pid_arg || *end != L'\0') {
            fwprintf(stderr, L"Invalid process id: %ls\n", pid_arg);
            return 2;
        }
        if (!absolute_existing_file(dll_arg, dll_path, MAX_PATH)) {
            fwprintf(stderr, L"DLL does not exist: %ls\n", dll_arg);
            return 2;
        }
        return inject_and_report((DWORD)process_id, dll_path);
    }
    if (pid_arg != NULL) {
        if (!absolute_existing_file(pid_arg, dll_path, MAX_PATH)) {
            fwprintf(stderr, L"DLL does not exist: %ls\n", pid_arg);
            return 2;
        }
    ...
    if (!default_dll_path(dll_path, MAX_PATH)) {
        fwprintf(stderr, L"UmbraNative.dll was not found beside the injector.\n");
        usage(argv[0]);
        return 2;
    }
    process_id = (unsigned long)select_process(dll_path);

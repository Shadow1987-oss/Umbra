...
extends HackModule {
    private final ToggleSetting instantSwap;
    private int originalSlot = -1;
    private final ToggleSetting swapBack;
    ...
    private final ToggleSetting swapWeapon = ToggleSetting.create(this, "Swap weapon", true, "Swaps to the strongest weapon on your hotbar");
    private final SliderSetting swapBackDelay;
    private final ToggleSetting requireMouseDown;
    private final SliderSetting swapToDelay;
    private final ToggleSetting onlyWhileSneaking;
    ...
    public void onTick(EventPrePlayerTick eventPrePlayerTick) {
        ...
        int currentSlot = eventPrePlayerTick.getThePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
        ...
        if (this.swapBackPending) {
            if (this.swapped && this.swapBackTimer.hasTimeElapsed(((Double)this.swapBackDelay.getValue()).longValue())) {
                if (this.originalSlot != -1) {
                    eventPrePlayerTick.getThePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.originalSlot);
                }
                this.swapped = false;
        ...
        if (this.swapPending) {
            ...
            if (shouldSwap) {
                ...
                if (bestSlot != -1 && bestSlot != currentSlot) {
                    eventPrePlayerTick.getThePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(bestSlot);
                    this.swapped = true;
                }
    ...
    public AutoTool() {
        ...
        this.swapToDelay = SliderSetting.create(this, "Swap to delay", "#", "ms", 0.0, 50.0, 500.0, 50.0, "How long to wait before swapping to tool");
        this.swapBack = ToggleSetting.create(this, "Swap back", false, "Swaps back to your original item when not hovering over blocks");
        this.swapBackDelay = SliderSetting.create(this, "Swap back delay", "#", "ms", 50.0, 350.0, 1000.0, 50.0, "How long to wait before swapping back");
        this.requireMouseDown = ToggleSetting.create(this, "Require mouse down", true, "Only swaps tools while holding left click");
        this.onlyWhileSneaking = ToggleSetting.create(this, "Only while sneaking", false, "Only swaps tools while sneaking");

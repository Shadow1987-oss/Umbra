...
import gg.umbra.hacks.exploits.blockin.BlockPlacementPathSegmentState;
...
import gg.umbra.tools.clutch.ClutchBlockPlacementPathSearchStrategy;
import gg.umbra.tools.clutch.ClutchPlacementPathUtils;
import gg.umbra.tools.clutch.ClutchSolidBlockPathSearchStrategy;
...
public class Clutch
extends HackModule {
    private final ToggleSetting keepSneak;
    ...
    private final ToggleSetting returnToLastSlot;
    ...
    private final ToggleSetting useBlacklist;
    private final RandomRangeSetting placeDelay;
    private PlacementTarget currentTarget;
    private final TimerUtil clickTimer;
    private final EnumFacing[] horizontalFacings = EnumFacing.c$src$ALgg_umbra_wrapper_impl_EnumFacing_$1i3g4ft();
    private final ToggleSetting silentAim;
    private final ToggleSetting sneak;
    private final TimerUtil aimTimer;
    private boolean paused;
    private final OptionSetting blockPriorityMode;
    private boolean sneaking;
    private final ToggleSetting bedFinder;
    private final EnumFacing[] verticalFacings = EnumFacing.t();
    private final ItemFilterList blockBlacklist;
    ...
    private boolean isUsableBlockSlot(EntityPlayerSP player, World world, int slot) {
        ...
        ItemStack itemStack = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().c(slot);
    ...
    public Clutch() {
        ...
        this.sneak = ToggleSetting.create(this, "Sneak", false, "Sneak when placing blocks");
        this.keepSneak = ToggleSetting.create(this, "Keep sneak", true, "Keeps sneak held after placing blocks.\nMust press sneak again to release");
        this.bedFinder = ToggleSetting.create(this, "Bed finder", true, "Finds nearby beds to block in towards");
        this.blockPriorityMode = OptionSetting.create((Object)this, "Block priority", "Lowest cost - Prioritizes by block type: Wool > Stone > Wood > Sandstone > Stained Clay > End Stone > Obsidian\nHardest - uses the block hardness value.", (ModeSelection)this.lowestCostMode, this.lowestCostMode, this.hardestMode);
        this.placeDelay = RandomRangeSetting.createWithDescription(this, "Place delay", "#", "ms", 0.0, 0.0, 30.0, 250.0, 1.0, "Delay between each block placement");
        this.aimSpeed = SliderSetting.create(this, "Aim speed", "#.#", "", 1.0, 12.0, 25.0, 0.1, "Speed of aim when placing blocks");
        this.returnToLastSlot = ToggleSetting.create(this, "Return to last slot", true, "Returns hotbar to previous slot when completed");
        this.useBlacklist = ToggleSetting.create(this, "Use blacklist", true, "BlockIn will not use these blocks");
        this.blockBlacklist = ItemFilterList.create(this, "blockin-blacklist", "Block blacklist", ItemFilterList.BLOCK_LIST_COLOR, ItemLimitData.DEFAULT_BLOCK_BLACKLIST);
    ...
    private void reset() {
        if (this.returnToLastSlot.getEffectiveValue().booleanValue() && this.previousSlot != -1) {
            ...
            if (localPlayer.isNotNull() && this.previousSlot != localPlayer.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v()) {
                localPlayer.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.previousSlot);
            }
            this.previousSlot = -1;
        }
    ...
    public void onTick(EventPreTick eventPreTick) {
        ...
        if (this.blockSlot != -1 && !this.placementNodes.isEmpty() && !this.paused) {
            ...
            if (!ClutchPlacementPathUtils.isPlacementSpaceClear(world, localPlayer, enclosureBlock)) {
                this.reset();
                Umbra.INSTANCE.getNotificationManager().show("Block-In Disabled", "Entity in the way!", NotificationType.WARNING, 2000L);
                return;
            }
            localPlayer.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.blockSlot);
            ...
            if (this.currentTarget != null) {
                ...
                if (!canPlaceImmediately) {
                    ...
                    if (this.aimTimer.hasTimeElapsed((long)this.placeDelay.getRandomRangeSetting() / 2L)) {
                        if (this.rotationController instanceof PointRotationController) {
                    ...
                    if (this.clickTimer.hasTimeElapsed((long)this.placeDelay.getRandomRangeSetting())) {
                        KeyBinding useKey = Minecraft.gameSettings().F();
                        if (useKey.u() || useKey.isPressed()) {
                            KeyBinding.setKeyBindState(useKey, false);
                        }
                        KeyBinding placementKey = Minecraft.gameSettings().b$src$Lgg_umbra_wrapper_impl_KeyBinding_$1yi3362();
                ...
                if (failedPlacements > 0) {
                    Umbra.INSTANCE.getNotificationManager().show("Block-In Disabled", "Failed to place " + failedPlacements + " block(s)!", NotificationType.WARNING, 2000L);
                } else if (nodesWithoutPaths > 0) {
                    Umbra.INSTANCE.getNotificationManager().show("Block-In Disabled", "No valid path found!", NotificationType.WARNING, 2000L);
                }
            }
        } else {
            ...
            Umbra.INSTANCE.getNotificationManager().show("Block-In Disabled", message, NotificationType.WARNING, 2000L);
            this.reset();
        }
    }
    ...
    private DirectionalPosition findClutchPosition(EntityPlayerSP player, World world) {
        ...
        DirectionalPosition playerPosition = new DirectionalPosition(player.z(), player.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().getMinY(), player.h(), 0);
        ...
        for (int yOffset = 0; yOffset > -2; --yOffset) {
            ...
            for (int facingOffset = 0; facingOffset < 4; ++facingOffset) {
                int playerFacing = player.J$src$Lgg_umbra_wrapper_impl_EnumFacing_$10aeq5x().c();
    ...
    private void computePlacementPaths(BlockData enclosureBlock, BlockPlacementNode node, EntityPlayerSP player, World world) {
        ...
        int startingFacing = player.J$src$Lgg_umbra_wrapper_impl_EnumFacing_$10aeq5x().c();
        ...
        if (!verticalPlacement) {
            ...
            int playerFacingIndex = player.J$src$Lgg_umbra_wrapper_impl_EnumFacing_$10aeq5x().c();
            ...
            EnumFacing upwardFacing = EnumFacing.F$src$Lgg_umbra_wrapper_impl_EnumFacing_$glfxl5();
            Block baseState = world.getBlockByPos(baseBlock.D(), baseBlock.B(), baseBlock.G());
    ...
    private int selectBestBlockSlot(EntityPlayerSP player, World world) {
        ...
        if (prioritizeHardness) {
            ...
            for (int i = 0; i < 9; ++i) {
                ...
                ItemStack itemStack = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().c(i);
    ...
    private boolean isStandingOnSolid(EntityPlayerSP player, World world) {
        ...
        int blockY = MathUtil.floor(player.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().getMinY() - 1.0);

...
import gg.umbra.tools.mlg.MLGBlockWrapper;
...
import gg.umbra.wrapper.impl.BlockStateWorldBridge;
...
import gg.umbra.wrapper.impl.Material;
...
extends HackModule {
    private final ToggleSetting refillRods;
    private final ToggleSetting recastGround;
    ...
    private final RandomRangeSetting clickDelay = RandomRangeSetting.createWithDescription(this, "Click delay", "#", "ms", 50.0, 75.0, 125.0, 200.0, 5.0, "How long to wait between clicks in the inventory");
    private boolean refillPending = false;
    private final ArrayDeque<KeyBinding> queuedKeys;
    private final ToggleSetting recastCaught;
    ...
    public void onTick(EventPreTick event) {
        ...
        if (!this.isHoldingRod()) {
            ...
            if (hotbarRodSlot != null && hotbarRodSlot.isNotNull()) {
                ...
                InventoryPlayer inventoryPlayer = localPlayer.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6();
            ...
            if (this.clickTimer.hasTimeElapsed((long)this.clickDelay.getRandomRangeSetting()) && (pendingClick = this.clickQueue.poll()) != null) {
        ...
        Entity caughtEntity = fishHook.r$src$Lgg_umbra_wrapper_impl_Entity_$18p7x3h();
    ...
    private EntityPlayerMacroBridge getFishHook() {
        ...
        EntityPlayerMacroBridge attachedHook = localPlayer.K$src$Lgg_umbra_wrapper_impl_EntityPlayerMacroBridge_$1agjn9();
        ...
        for (Object handle : world.z()) {
            ...
            if (!localPlayer.equals(hook.A$src$Lgg_umbra_wrapper_impl_Entity_$12ijiu4())) continue;
            ownedHooks.add(hook);
        }
    ...
    private boolean isHookInBlock(EntityPlayerMacroBridge fishHook, WorldClient world) {
        BlockPos blockPos = fishHook.J$src$Lgg_umbra_wrapper_impl_BlockPos_$kv8a0x();
    ...
    private boolean isHookInWater(EntityPlayerMacroBridge fishHook, WorldClient world) {
        ...
        for (int sample = 0; sample < sampleCount; ++sample) {
            AxisAlignedBB hookBounds = fishHook.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu();
    ...
    private void recast() {
        ...
        if (hasActiveHook || !this.hasCast || this.castTimer.hasTimeElapsed(1000L)) {
            this.pressUseKey(Minecraft.gameSettings().b$src$Lgg_umbra_wrapper_impl_KeyBinding_$1yi3362());
            this.castTimer.reset();
            this.groundTimer.reset();
            this.stationaryTimer.reset();
            this.hasCast = true;
    ...
    public AutoFish() {
        ...
        this.recastCaught = ToggleSetting.create(this, "Recast caught", false, "Automatically recasts if the hook catches onto an entity");
        this.recastGround = ToggleSetting.create(this, "Recast ground", false, "Automatically recasts if the hook hits the ground");
        ...
        this.refillRods = ToggleSetting.create(this, "Refill rods", true, "Automatically replaces broken rods with rods from your inventory.");

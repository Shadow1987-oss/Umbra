...
public class NotificationSoundPlayer {
    ...
    public boolean isMuted() {
        return Umbra.INSTANCE.getPublicProfileSettings().muted.getEffectiveValue();
    }
    ...
    public float getVolumePercent() {
        return ((Double)Umbra.INSTANCE.getPublicProfileSettings().volume.getValue()).floatValue();
    }
    ...
    public void startSoundThread() {
        new Thread(this::runSoundLoop, "Umbra notification sound player").start();
    }

    private void runSoundLoop() {
        while (!Umbra.INSTANCE.enabled) {
            try {
                Thread.sleep(100L);

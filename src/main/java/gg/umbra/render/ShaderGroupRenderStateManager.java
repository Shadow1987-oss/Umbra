...
import gg.umbra.wrapper.impl.ShaderGroup;
...
public class ShaderGroupRenderStateManager {
    ...
    public void enable() {
        ...
        if (ForgeVersion.MC_1_16_5.d() && ForgeVersion.MC_1_16_5_ACTUAL.B() && Umbra.INSTANCE.isForgeAbsent() && !Umbra.INSTANCE.isVanillaMinecraftPresent()) {
            return;
        }
        ...
        if (shadersEnabled && NativeBridge.isForgeAbsent() && !Umbra.INSTANCE.isVanillaMinecraftPresent()) {
            return;
        }
    ...
    private void createShaderGroup(int displayWidth, int displayHeight) {
        this.activeShaderGroup = ShaderGroup.create(Minecraft.getTextureManager(), Minecraft.m$src$Lgg_umbra_wrapper_impl_EntityRenderer_$13begmf().K(), Minecraft.getFrameBuffer(), new ResourceLocation(EntityRenderer.getShaderResourceLocations()[18]));
        Minecraft.m$src$Lgg_umbra_wrapper_impl_EntityRenderer_$13begmf().J(this.activeShaderGroup);
        this.activeShaderGroup.resize(displayWidth, displayHeight);
        Minecraft.m$src$Lgg_umbra_wrapper_impl_EntityRenderer_$13begmf().setUseShader(true);
    }
}

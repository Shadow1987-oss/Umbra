...
import gg.umbra.utils.NanoTimerUtil;
...
import gg.umbra.wrapper.impl.PlayerInteractEventAction;
import gg.umbra.wrapper.impl.PlayerPositionLookPacketModern;
import gg.umbra.wrapper.impl.RayTraceResult;
import gg.umbra.wrapper.impl.S08PacketPlayerPosLook;
...
implements EventListener {
    ...
    public float adjustMovementYawFromBindings(float yaw, boolean useCurrentKeyState) {
        ...
        KeyBinding leftKey = settings.g$src$Lgg_umbra_wrapper_impl_KeyBinding_$qqn5n3();
        KeyBinding rightKey = settings.x$src$Lgg_umbra_wrapper_impl_KeyBinding_$1cf7isg();
    ...
    public void onRender2D(EventRender2D event) {
        ...
        if (!Umbra.INSTANCE.getClientSettings().aimIndicator.getEffectiveValue().booleanValue()
    ...
    private void refreshReachRayTraces() {
        double reachDistance = Umbra.INSTANCE.getHackManager().getMod(Reach.class).getReachDistance();
        double hitBoxExpansion = Umbra.INSTANCE.getHackManager().getMod(HitBoxes.class).getExpansionAmount();
    ...
    public void onPreRenderWorldPass(EventPreRenderWorldPass eventPreRenderWorldPass) {
        if (!Umbra.INSTANCE.getClientSettings().thirdPersonAimView.getEffectiveValue().booleanValue()) {
            return;
        }
    ...
    private void applyManagedMouseOver() {
        if (this.cachedMouseOverRayTrace == null || this.cachedMouseOverRayTrace.isNull()) {
            ...
            if (Umbra.INSTANCE.getClientSettings().useReach.getEffectiveValue().booleanValue()) {
                reachDistance = Umbra.INSTANCE.getHackManager().getMod(Reach.class).getReachDistance();
            }
            double hitBoxExpansion = 0.0;
            if (Umbra.INSTANCE.getClientSettings().useHitboxes.getEffectiveValue().booleanValue()) {
                hitBoxExpansion = Umbra.INSTANCE.getHackManager().getMod(HitBoxes.class).getExpansionAmount();
            }
            MouseOverRayTraceUpdater.s((float)reachDistance, (float)hitBoxExpansion);
            this.cachedMouseOverRayTrace = Minecraft.p$src$Lgg_umbra_wrapper_impl_RayTraceResult_$5rw6n0();
            player.H(savedYaw);
    ...
    public void onPreLocalPlayerTick(EventPreLocalPlayerTick event) {
        ...
        ModeSelection movementMode = (ModeSelection)Umbra.INSTANCE.getClientSettings().movementCorrection.getValue();
        ...
        if (shouldRemapMovement) {
            ...
            this.savedLeftKeyState = settings.g$src$Lgg_umbra_wrapper_impl_KeyBinding_$qqn5n3().isKeyDown();
            this.savedRightKeyState = settings.x$src$Lgg_umbra_wrapper_impl_KeyBinding_$1cf7isg().isKeyDown();
            this.savedBackKeyState = settings.s().isKeyDown();
            settings.Y().setPressed(pressForward);
            settings.g$src$Lgg_umbra_wrapper_impl_KeyBinding_$qqn5n3().setPressed(pressLeft);
            settings.x$src$Lgg_umbra_wrapper_impl_KeyBinding_$1cf7isg().setPressed(pressRight);
    ...
    private boolean hasMovementKeyStateMismatch(GameSettings settings) {
        KeyBinding[] movementKeys = new KeyBinding[]{
                settings.Y(),
                settings.g$src$Lgg_umbra_wrapper_impl_KeyBinding_$qqn5n3(),
                settings.x$src$Lgg_umbra_wrapper_impl_KeyBinding_$1cf7isg(),
                settings.s()
        };
    ...
    private void restoreMovementKeyStates() {
        ...
        MovementInputHelper.synchronizeKeyState(settings.g$src$Lgg_umbra_wrapper_impl_KeyBinding_$qqn5n3(), this.savedLeftKeyState);
        MovementInputHelper.synchronizeKeyState(settings.x$src$Lgg_umbra_wrapper_impl_KeyBinding_$1cf7isg(), this.savedRightKeyState);

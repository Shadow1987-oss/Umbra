...
extends Block {
    public static BlockProperty part() {
        return new BlockProperty(MBlockBed.getPart(BlockBed.umbraInstance.getMappings().Dj));
    }
    ...
    public boolean isFoot(World world, int blockX, int blockY, int blockZ) {
        ...
        BlockPos blockPos = BlockPos.create(blockX, blockY, blockZ);
        BlockState blockState = world.getBlockState(blockPos);
        BlockState actualState = new BlockState(BlockBed.umbraInstance.getMappings().qg.y(this.I, blockState.getObject(), world.getObject(), blockPos.getObject()));
    ...
    public EnumFacing getBedDirection(World world, int blockX, int blockY, int blockZ) {
        if (ForgeVersion.MC_1_16_5.d()) {
            BlockPos blockPos = BlockPos.create(blockX, blockY, blockZ);
            Object direction = MBlockBed.getBedDirection(BlockBed.umbraInstance.getMappings().Dj, world.getObject(), blockPos.getObject());

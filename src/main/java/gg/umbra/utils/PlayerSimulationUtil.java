...
public class PlayerSimulationUtil {
    public static EntityOtherPlayerMP I(EntityPlayer entityPlayer) {
        EntityOtherPlayerMP entityOtherPlayerMP = EntityOtherPlayerMP.create(Minecraft.theWorld(), entityPlayer.c$src$Lgg_umbra_wrapper_impl_GameProfile_$ir8937());
    ...
    public static void t(EntityPlayer entityPlayer, boolean bl) {
        ...
        PlayerSimulationUtil.p(entityPlayer, entityPlayer.z() - (double)entityPlayer.f$src$F$fst3ac() * 0.35, entityPlayer.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu().getMinY() + 0.5, entityPlayer.h() + (double)entityPlayer.f$src$F$fst3ac() * 0.35);
        PlayerSimulationUtil.p(entityPlayer, entityPlayer.z() - (double)entityPlayer.f$src$F$fst3ac() * 0.35, entityPlayer.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu().getMinY() + 0.5, entityPlayer.h() - (double)entityPlayer.f$src$F$fst3ac() * 0.35);
        PlayerSimulationUtil.p(entityPlayer, entityPlayer.z() + (double)entityPlayer.f$src$F$fst3ac() * 0.35, entityPlayer.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu().getMinY() + 0.5, entityPlayer.h() - (double)entityPlayer.f$src$F$fst3ac() * 0.35);
        PlayerSimulationUtil.p(entityPlayer, entityPlayer.z() + (double)entityPlayer.f$src$F$fst3ac() * 0.35, entityPlayer.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu().getMinY() + 0.5, entityPlayer.h() + (double)entityPlayer.f$src$F$fst3ac() * 0.35);
    ...
    private static boolean E(BlockPos blockPos) {
        ...
        if (ForgeVersion.MC_1_12_2.d()) {
            blockState2 = world.getBlockState(blockPos.X$src$Lgg_umbra_wrapper_impl_BlockPos_$jlnp6b());
        }
        return !world.getBlockState(blockPos).getBlock().X(blockState) && !world.getBlockState(blockPos.X$src$Lgg_umbra_wrapper_impl_BlockPos_$jlnp6b()).getBlock().X(blockState2);
    }
    ...
    public static void p(EntityPlayer entityPlayer, double d, double d2, double d3) {
        ...
        if (!PlayerSimulationUtil.E(blockPos)) {
            ...
            if (PlayerSimulationUtil.E(blockPos.o$src$Lgg_umbra_wrapper_impl_BlockPos_$10np7re()) && d5 < d6) {
                d6 = d5;
                n = 4;
            }
    ...
    public static void g(EntityPlayer entityPlayer, float f, float f2) {
        ...
        if (!entityPlayer.h$src$Z$ftwoya() || entityPlayer.C$src$Lgg_umbra_wrapper_impl_ModelPlayer_$19uhx86().isFlying()) {
            if (!entityPlayer.Q$src$Z$fh9faz() || entityPlayer.C$src$Lgg_umbra_wrapper_impl_ModelPlayer_$19uhx86().isFlying()) {
                boolean bl;
                float f3 = 0.91f;
                if (entityPlayer.b$src$Z$fqlxe4()) {
                    if (ForgeVersion.MC_1_7_10.Y()) {
                        f3 = world.getBlockState(BlockPos.create(MathUtil.floor(entityPlayer.z()), MathUtil.floor(entityPlayer.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu().getMinY()) - 1, MathUtil.floor(entityPlayer.h()))).getBlock().c() * 0.91f;
                    } else {
                        BlockData blockData = new BlockData(MathUtil.floor(entityPlayer.z()), MathUtil.floor(entityPlayer.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().getMinY()) - 1, MathUtil.floor(entityPlayer.h()));
    ...
    public static void s(EntityPlayer entityPlayer, EntityPlayer entityPlayer2) {
        ...
        entityPlayer.D(entityPlayer2.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().expand(0.0, 0.0, 0.0));

...
public class ItemStackFingerprint {
    ...
    public static ItemStack[] T$src$ALgg_umbra_wrapper_impl_ItemStack_$f6ukg1(EntityPlayer entityPlayer) {
        boolean bl = ForgeVersion.MC_1_12_2.d();
        if (bl) {
            ItemStack[] itemStackArray = new ItemStack[6];
            ItemStack itemStack = entityPlayer.B$src$Lgg_umbra_wrapper_impl_ItemStack_$impdvt();
            if (itemStack.isNotNull()) {
                itemStackArray[0] = itemStack;
            }
            Object[] objectArray = entityPlayer.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().i();
    ...
    public static long T(EntityPlayer entityPlayer) {
        return ItemStackFingerprint.t(ItemStackFingerprint.T$src$ALgg_umbra_wrapper_impl_ItemStack_$f6ukg1(entityPlayer));
    }
}

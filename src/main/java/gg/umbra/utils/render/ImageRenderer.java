...
import gg.umbra.unmap.ImageParser$Format;
...
import gg.umbra.utils.render.TextureAtlas;
import gg.umbra.utils.render.TextureAtlasRegion;
...
public class ImageRenderer {
    ...
    public static double getImageHeight(String resourceName) {
        if (GuiRenderPrimitives.d()) {
            TextureAtlas textureAtlas = TextureAtlasRegistry.getInstance().get("umbra_texture");
    ...
    public static GlImageTexture loadResource(String resourceName, boolean useMipmaps, boolean whiteMask) {
        ...
        try {
            ...
            byte[] resourceData = Umbra.readResource(resourcePath);
            if (resourceData == null || resourceData.length == 0) {
                if ("world".equals(resourceName)) {
                    return null;
                }
                Umbra.debugLog("IMG missing: " + resourcePath + " (falling back to world)");
    ...
    public static void preloadResources() {
        ImageRenderer.loadResource("umbralogo", true, false);

...
import gg.umbra.value.ToggleableListEntry;
...
extends AbstractListValueComponent {
    ...
    private void openEditor() {
        Frame frame;
        if (this.getParentFrameComponent() != null && (frame = this.getParentFrameComponent().L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa()) instanceof ClickGuiMainFrame) {
    ...
    private boolean hasSingleBooleanParentEntry() {
        if (!(this.L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa() instanceof ClickGuiMainFrame)) {
            return false;
        }
        if (!(this.listValue.getParent() instanceof ToggleSetting)) {
            return false;
        }
        return ((ToggleSetting)this.listValue.getParent()).getDependentValues().size() == 1;
    }

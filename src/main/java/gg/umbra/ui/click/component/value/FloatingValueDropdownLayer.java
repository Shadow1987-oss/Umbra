...
import gg.umbra.ui.click.component.value.AbstractListValueComponent;
...
extends Frame {
    ...
    public void updatePosition() {
        ...
        double preferredX = ((GuiComponent)this.sourceComponent).L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().G$src$D$1b2f02a() + ((GuiComponent)this.sourceComponent).L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().A() + 1.0;
        if (preferredX != this.lastSourceX || ((GuiComponent)this.sourceComponent).n() != this.lastSourceY) {
            ...
            if (frameComponent.k$src$Z$if6xeb()) {
                ...
                targetY = Math.max(targetY, frameComponent.n() + (frameComponent.j$src$Lgg_umbra_ui_click_frame_FrameHeaderComponent_$175vsfc() != null ? frameComponent.j$src$Lgg_umbra_ui_click_frame_FrameHeaderComponent_$175vsfc().L() : 0.0));
            }
            double screenCenterX = Minecraft.J() / 2;
            if (preferredX + this.A() / 2.0 > screenCenterX) {
                this.M(preferredX - ((GuiComponent)this.sourceComponent).L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().A() - this.A() - 2.0, targetY);
            } else {
    ...
    public void Y() {
        ...
        ClientSettings.INSTANCE.replaceFrame(((GuiComponent)this.sourceComponent).getParentFrameComponent().L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa(), this);
    }

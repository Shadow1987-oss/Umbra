...
import gg.umbra.ui.click.component.TooltipTextSegment;
...
extends SimpleTextLabelComponent {
    ...
    @Override
    public void H() {
        if (!this.targetComponent.w$src$Z$e457mb() || !this.targetComponent.t() || !this.targetComponent.V$src$Z$1xhop3l() || this.targetComponent.L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa() != null && !this.targetComponent.L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().i$src$Z$1f7f2w6()) {
        ...
        String formattedText = Umbra.INSTANCE.getFontSelector().W().s(this.text);
        ...
        if (tooltipX + contentWidth + 8.0 > (double)Minecraft.J() / Umbra.INSTANCE.getClientSettings().getGuiScaleFactor() / 2.0) {
            tooltipX = tooltipX - contentWidth - 6.0 - 12.0;
        }
        double tooltipY = this.n();
        if (tooltipY + contentHeight + 2.5 + 2.0 > (double)Minecraft.h() / Umbra.INSTANCE.getClientSettings().getGuiScaleFactor() / 2.0) {

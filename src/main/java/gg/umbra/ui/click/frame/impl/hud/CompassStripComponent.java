...
extends GuiComponent {
    ...
    private void renderCompass() {
        ...
        double tickSpacing = (this.frame.A() + 4.0) / 1400.0
                * Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
        double tickX = this.frame.G$src$D$1b2f02a() * Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
        ...
        SmoothFontRenderer smoothFontRenderer = Umbra.INSTANCE.getFontManager().W(0.8, false);
    ...
    private void drawTick(double x, float heading) {
        double frameY = this.frame.n() * Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
        double frameHeight = this.frame.L() * Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
        if (heading < 0.0f) {
    ...
    private void drawLabel(String text, double x, double y, Color color, boolean emphasized) {
        ...
        SmoothFontRenderer smoothFontRenderer = emphasized
                ? Umbra.INSTANCE.getFontManager().W(0.7, false)
                : Umbra.INSTANCE.getFontManager().Y(0.7);
        if (this.frame.shouldRenderHudBackground()) {

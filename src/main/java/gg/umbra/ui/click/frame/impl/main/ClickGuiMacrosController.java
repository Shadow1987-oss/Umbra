...
import gg.umbra.ui.click.component.MacroCardComponent;
import gg.umbra.ui.click.component.MultilineTextBlockComponent;
...
import gg.umbra.ui.click.frame.impl.main.ClickGuiContentPanel;
import gg.umbra.ui.click.frame.impl.main.ClickGuiMacrosLabelInput;
...
public class ClickGuiMacrosController {
    ...
    private void deleteMacro(Macro macro) {
        Umbra.INSTANCE.getMacrosManager().removeMacro(macro);
        this.modulesPage.mainFrame.closeActiveOverlay();
        this.rebuildMacroCards();
    }
    ...
    public void renderMacroView() {
        ...
        this.searchInput = new ClickGuiMacrosLabelInput(this, "BlockFinder macros...");
        ...
        double contentHeight = this.modulesPage.getMainContent().L() - this.modulesPage.getMainContent().l$src$Lgg_umbra_ui_click_layout_ComponentLayout_$di1tij().y() - 1.0;
        if (contentHeight < 0.0) {
            contentHeight = 0.0;
        }
        this.macroContent = new ClickGuiContentPanel(availableWidth, contentHeight);
        this.macroContent.l$src$Lgg_umbra_ui_click_layout_ComponentLayout_$di1tij().M("wrap");
    ...
    public void rebuildMacroCards() {
        ...
        if (hasSearchQuery) {
            for (Macro macro : Umbra.INSTANCE.getMacrosManager().getMacros()) {

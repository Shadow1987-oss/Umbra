...
import gg.umbra.ui.click.frame.impl.profile.ProfilesShowActiveRowsClickHandler;
import gg.umbra.ui.click.frame.impl.profile.ProfilesShowAllRowsClickHandler;
...
extends SettingsSubpageFrame {
    ...
    public void closePopupAndDiscardDraft() {
        if (this.activePopup != null) {
            this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().restoreDefaultNavigation();
            ClientSettings.removePopup(this.activePopup);
            this.activePopup = null;
        }
        if (this.createPanel.getPendingProfile() != null) {
            Umbra.INSTANCE.getProfilesManager().setActiveProfile(this.createPanel.getPendingProfile());
    ...
    public void c() {
        ...
        for (int i = 0; i < 5; ++i) {
            this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().updateLayout();
        }
        this.profileList.o(110.0);
        super.c();
        if (!this.showingAllProfiles && !this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().isShowingBackNavigation() && hiddenProfileCount > 0 && this.w$src$Z$e457mb()) {
            ...
            font.d(countText, this.G$src$D$1b2f02a() + this.A() - 40.0 - textWidth, this.n() + this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().L() / 2.0 - textHeight / 2.0, ProfilesSettingsFrame.J.Z);
        }
    }
    ...
    public ProfilesSettingsFrame() {
        ...
        this.N(false);
        this.D(true);
        this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().setDefaultIconScale(0.5f);
        for (GuiComponent guiComponent : ProfilesSettingsFrameState.F(false)) {
            BooleanToggleComponent booleanToggleComponent;
            this.n(guiComponent);
            if (!(guiComponent instanceof BooleanToggleComponent) || !(booleanToggleComponent = (BooleanToggleComponent)guiComponent).getBoundValue().equals(Umbra.INSTANCE.getPublicProfileSettings().autoLoadModuleStates)) continue;
        ...
        this.contentLayout.l$src$Lgg_umbra_ui_click_layout_ComponentLayout_$di1tij().M("wrap");
        ...
        this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().addAction(this.editHiddenProfilesButton);
        this.i$src$Lgg_umbra_ui_click_frame_FrameToolbarComponent_$gnpgc6().addAction(this.doneButton);
    }

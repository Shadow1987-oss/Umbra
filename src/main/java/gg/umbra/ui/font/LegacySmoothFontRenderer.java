...
import gg.umbra.ui.font.SmoothFontRasterState;
...
extends SmoothFontRenderer {
    ...
    public LegacySmoothFontRenderer(String string, int n) {
        ...
        for (String string2 : FontManager.y) {
            ...
            try {
                Umbra.debugLog("Found file attempting to read: " + string);
    ...
    public void O(String string, double d, double d2, int n, boolean bl) {
        if (this.j) {
            string = Umbra.INSTANCE.getFontSelector().W().s(string);
        }
        if (!bl) {
            ...
            string = string2;
            double d3 = Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
            float f = 0.6f;
            if (d3 != 1.0) {
                f *= (float)(1.0 / d3);
            }
    ...
    private void B(byte[] byArray, int n, String string) {
        ...
        hashSet.add(0);
        byte[] byArray2 = Umbra.readResource("chinese.properties.txt");
        if (byArray2 != null) {
            try {
        ...
        if (SmoothFontRasterState.z(stbTrueTypeFontInfo, byArray, 0) == 0) {
            Umbra.debugLog("Failed to init font information");
            return;
        }
        ...
        if (SmoothFontRasterState.C(stbTrueTypePackContext, byArray3, n3, n3, 0, 1) == 0) {
            Umbra.debugLog("Failed to begin packing");
            return;
        }
        int n4 = 2;
        int n5 = 2;

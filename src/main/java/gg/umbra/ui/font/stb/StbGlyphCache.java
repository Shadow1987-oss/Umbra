...
import gg.umbra.ui.font.SmoothFontGlyph;
import gg.umbra.ui.font.stb.StbGlyphCacheEntry;
import gg.umbra.wrapper.impl.FontManager;
import gg.umbra.wrapper.impl.FontSet;
import gg.umbra.wrapper.impl.ForgeVersion;
import gg.umbra.wrapper.impl.GlyphInfo;
...
public class StbGlyphCache {
    ...
    public void U(FontSet fontSet) {
        if (fontSet.isNull()) {
            Umbra.debugLog("[MCFontSetup] Failed to initialize - FontSet is null");
            return;
        }
        this.V = fontSet;
        this.U = true;
        this.I.clear();
        this.h.clear();
        Umbra.debugLog("[MCFontSetup] Initialized with FontSet");
    }
    ...
    private boolean l(int n) {
        ...
        try {
            ...
            Umbra.debugLog("[MCFontSetup] Error detecting texture format for ID " + n + ": " + exception.getMessage());
        }
        this.h.put(n, bl2);
        return bl2;
    }
    ...
    private StbGlyphCacheEntry B(int n) {
        try {
            ...
            int n2 = glyphInfo.q();
            if (n2 <= 0) {
                Umbra.debugLog("[MCFontSetup] Invalid texture ID for codepoint " + n);
                return null;
            }
            ...
            Umbra.debugLog("[MCFontSetup] Error extracting glyph for codepoint " + n + ": " + exception.getMessage());
            return null;
        }
    }
    ...
    public boolean l() {
        if (!ForgeVersion.MC_1_21_10.d()) {
            Umbra.debugLog("[MCFontSetup] Minecraft font bridge requires 1.21.10+");
            return false;
        }
        try {
            FontManager fontManager = Minecraft.q();
            if (fontManager == null) {
                Umbra.debugLog("[MCFontSetup] FontManager is null");
                return false;
            }
            FontSet fontSet = fontManager.h();
            if (fontSet == null || fontSet.getObject() == null) {
                Umbra.debugLog("[MCFontSetup] Default FontSet is null");
                return false;
            }
            this.U(fontSet);
            return true;
            ...
            Umbra.debugLog("[MCFontSetup] Error initializing from Minecraft: " + exception.getMessage());

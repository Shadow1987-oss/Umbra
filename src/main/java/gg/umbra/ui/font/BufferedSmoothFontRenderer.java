...
extends SmoothFontRenderer {
    ...
    private void u(double d, double d2, String string, int n, boolean bl, float f, boolean bl2) {
        SmoothFontRenderer smoothFontRenderer = Umbra.INSTANCE.getFontManager().x(this.D);
    ...
    public double R(String string, boolean bl) {
        if (!bl) {
            float f = 0.6f;
            float f2 = (float)Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
            if ((double)f2 != 1.0) {
                f *= 1.0f / f2;
            }
    ...
    private void w(byte[] byArray, int n, String string) {
        ...
        block20: {
            ...
            byte[] byArray2 = Umbra.readResource(string2);
            if (byArray2 != null) {
                try {

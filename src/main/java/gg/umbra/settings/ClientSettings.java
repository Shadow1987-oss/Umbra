...
import gg.umbra.hacks.exploits.invwalk.InvWalkSettingsState;
...
import gg.umbra.tools.inventory.cleaner.ui.InventoryFilterRuleEditorFrame;
...
import gg.umbra.ui.click.component.PopupMenuButtonComponent;
import gg.umbra.ui.click.component.TextInputComponentBase;
import gg.umbra.ui.click.component.ToolTips;
...
import gg.umbra.ui.click.frame.impl.hud.HudModuleFrameBase;
...
import gg.umbra.ui.click.frame.impl.profile.ProfileSnapshotFrame;
...
import gg.umbra.utils.render.BlurRegionRenderer;
...
extends HackModule {
    ...
    public final ToggleSetting multiKeybinding = ToggleSetting.create((Object)((Object)this), (String)"Enable Multi-Keybinding", (boolean)false, (String)"Allows you to set multiple keys to be held together to toggle modules");
    private FrameStackManager activeStack;
    public final SliderSetting rainbowSpeed;
    public final OptionSetting language;
    ...
    public final OptionSetting searchBarStyle;
    ...
    private void applyStandaloneMode(PublicProfileSettings publicProfileSettings) {
        if (this.activeStack == mainStack || this.activeStack == clickGuiFrameManager) {
            if (publicProfileSettings.centralGuiStyle.isSelected()) {
                ...
                Umbra.debugLog("GUI style: switched to central GUI");
            } else {
                if (this.activeStack == mainStack) {
                    return;
                }
                this.switchFrameStack(mainStack);
                Umbra.debugLog("GUI style: switched to frames GUI");
            }
        }
    }
    ...
    public static void refreshCategoryHeader(Category category) {
        for (Frame frame : allFrames) {
            if (!((ModeSelection)ClientSettings.INSTANCE.searchBarStyle.getValue()).equals(ClientSettings.INSTANCE.integratedSearchBarMode) && frame instanceof ClientSettingsSearchFrame || !(frame instanceof ModuleCategoryFrame) || !((ModuleCategoryFrame)frame).G$src$Lgg_umbra_module_Category_$qyt4o7().equals(category)) continue;
            int enabledModuleCount = ((ModuleCategoryFrame)frame).A$src$I$wwnvku();
            if (!(frame.j$src$Lgg_umbra_ui_click_frame_FrameHeaderComponent_$175vsfc() instanceof ModuleCategoryFrameHeader)) continue;
            ((ModuleCategoryFrameHeader)frame.j$src$Lgg_umbra_ui_click_frame_FrameHeaderComponent_$175vsfc()).y(enabledModuleCount);
        }
    }
        ...
        if ((KeyboardInput.isKeyDown((int)163) || KeyboardInput.isKeyDown((int)162)) && keyCode == 70 && keyDown) {
            ...
            if (!this.noSearchBarMode.isSelected()) {
                if (this.floatingSearchBarMode.isSelected()) {
                    moduleSearchFrame.n$src$Lgg_umbra_ui_click_frame_impl_ModuleSearchFrameHe$xia8v2().s();
                    moduleSearchFrame.n$src$Lgg_umbra_ui_click_frame_impl_ModuleSearchFrameHe$xia8v2().A$src$Lgg_umbra_ui_click_component_input_ModuleSearchIn$1efzz7n().requestFocus();
    ...
    public void toggle() {
        ...
        if (this.isEnabled()) {
            ...
            Umbra.debugLog((String)"Gui Closed 2");
            this.inputEnabled = true;
        }
    }

    private static void onSearchBarStyleChanged(OptionSetting ignoredValue) {
        settingsSearchFrame.resetSearchAsync();
    }

    public void renderFramesAndNotifications() {
        this.updateFrames();
        NotificationManager notificationManager = Umbra.INSTANCE.getNotificationManager();
    ...
    private void dispatchMouseButton(int button) {
        ...
        if (!(this.activeStack instanceof ClickGuiFrameManager)) {
            for (Frame frame : framesInInputOrder) {
                if (!frame.V$src$Z$1xhop3l() || !frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().V$src$Z$1xhop3l() || !frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().w$src$Z$e457mb()) continue;
                frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().dispatchPrimaryClick();
                return;
            }
        }
    ...
    public void updateGuiScale() {
        Umbra.INSTANCE.getClientSettings().guiColor.applyConfiguredColorTransform();
        double currentGuiScale = Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
    ...
    public static void removePopup(PopupFrame popupFrame) {
        ...
        popupFrame.Q$src$Lgg_umbra_ui_click_frame_Frame_$1y8ivjg().s$src$Ljava_util_ArrayList_$1a2240q().remove(popupFrame);
    }

    public void syncRainbowHue() {
        MutableColor mutableColor = Umbra.INSTANCE.getClientSettings().guiColor.getMutableColor();
    ...
    public static void removeFramePopups(Frame frame) {
        ...
        frame.L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().s$src$Ljava_util_ArrayList_$1a2240q().clear();
    }
    ...
    public void renderHudFrames() {
        ...
        if (INSTANCE.isInputEnabled()) {
            ...
            double guiScale = Umbra.INSTANCE.getClientSettings().getGuiScaleFactor();
    ...
    private void onStandaloneModeChanged(OptionSetting modeValue) {
    ...
    private void updateFrames() {
        ...
        for (Frame frame : forwardRenderOrder) {
            if (frame.V$src$Z$1xhop3l() && frame.a$src$Z$1f30q5a() && !(this.activeStack instanceof ClickGuiFrameManager)) {
                frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().setVisible(true);
                frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().c();
                if (!frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().t()) continue;
                frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().J();
                continue;
            }
            frame.t$src$Lgg_umbra_ui_click_component_gui_TextButton_$p5ute3().setVisible(false);
        }
    ...
    public Color getAccentColor() {
        ...
        return Umbra.INSTANCE.getClientSettings().guiColor.getMutableColor();
    }
    ...
    public static void initializeFrames() {
        ...
        allFrames.clear();
        Umbra.INSTANCE.initializeRender();
        ...
        Umbra umbra = Umbra.INSTANCE;
        if (umbra.isFeatureDisabled()) {
    ...
    public static <T extends PopupFrame> T createPopup(GuiComponent owner, GuiComponent content, Class<T> popupClass) {
        try {
            ...
            owner.L$src$Lgg_umbra_ui_click_frame_Frame_$1djx6sa().s$src$Ljava_util_ArrayList_$1a2240q().add(popupFrame);
            return (T)popupFrame;
        }
        catch (Exception exception) {
            Umbra.logThrowable((Throwable)exception);
            return null;
        }
    }
    ...
    public void onDisable() {
        ...
        InvWalkSettingsState.setPressed((KeyBinding)Minecraft.gameSettings().y$src$Lgg_umbra_wrapper_impl_KeyBinding_$1hvjjoh(), (boolean)false);
    }
    ...
    /**
     * While the GUI is open, makes sure the active frame stack matches the
     * selected "GUI style". Without this, some paths (e.g. leaving the session
     * spoof screen) can leave the active stack on the frames GUI even when
     * the user has the central GUI selected.
     */
    private void autoHealStandaloneMode() {
        if (this.inputEnabled) {
            return;
        }
        PublicProfileSettings publicProfileSettings = Umbra.INSTANCE.getPublicProfileSettings();
        if (publicProfileSettings == null || publicProfileSettings.guiStyle.isPersistenceSuppressed()) {
            return;
        }
        if (publicProfileSettings.centralGuiStyle.isSelected() && this.activeStack != null && !(this.activeStack instanceof ClickGuiFrameManager)) {
            this.applyStandaloneMode(publicProfileSettings);
        } else if (publicProfileSettings.framesGuiStyle.isSelected() && this.activeStack instanceof ClickGuiFrameManager) {
            this.applyStandaloneMode(publicProfileSettings);
        }
    }
    ...
    protected Bendable C$src$Lgg_umbra_unmap_Bendable_$1we4j6l() {
        return new ClientSettingsGuiBindBendable((HackModule)this);
    }
    ...
    public ClientSettings() {
        ...
        this.smoothFont = new ClientSettingsToggleSetting((Object)this, "Smooth Font", true);
        this.showTooltips = ToggleSetting.create((Object)((Object)this), (String)"Show tooltips", (boolean)true, (String)"Toggles visibility of these");
        this.rainbowSpeed = SliderSetting.create((Object)((Object)this), (String)"Rainbow speed", (String)"#.#", (String)"", (double)0.1, (double)1.0, (double)10.0, (double)0.1, (String)"Adjusts the speed of rainbow values");
        this.language = (OptionSetting)OptionSetting.create((Object)((Object)this), (String)"Language", (ModeSelection)FontSelector.j, (ModeSelection[])new ModeSelection[]{FontSelector.j, FontSelector.S, FontSelector.c, FontSelector.a, FontSelector.P}).setResettable(false);
        this.showLegitMode = ToggleSetting.create((Object)((Object)this), (String)"Show legit mode", (boolean)true, (String)"Shows the button to switch to the legit mod menu");
        this.showEnabledCount = ToggleSetting.create((Object)((Object)this), (String)"Show enabled count", (boolean)true, (String)"Shows the number of enabled modules in the standalone gui");
        ...
        this.searchBarStyle = OptionSetting.create((Object)((Object)this), (String)"BlockFinder bar style", (String)"Switch between search bar styles", (ModeSelection)this.floatingSearchBarMode, (ModeSelection[])new ModeSelection[]{this.integratedSearchBarMode, this.noSearchBarMode, this.floatingSearchBarMode});
        PublicProfileSettings publicProfileSettings = Umbra.INSTANCE.getPublicProfileSettings();
    ...
    public void disableBlurShader() {
        if (((ClientSettings)Umbra.INSTANCE.getHackManager().getMod(ClientSettings.class)).blurBackground.getEffectiveValue().booleanValue()) {
            ShaderGroupRenderStateManager.getInstance().disable();
        }
    }
    ...
    /**
     * Lightweight per-tick hook that only keeps the "GUI style" selection in
     * sync with the active frame stack. Runs on every pre-tick through
     * ClientSettingsEventForwarder (the full onTick body is not used because
     * some of its branches assume state this runtime never had).
     */
    public void tickStandaloneMode() {
        if (!this.initialized) {
            this.initialized = true;
            PublicProfileSettings publicProfileSettings = Umbra.INSTANCE.getPublicProfileSettings();
            if (publicProfileSettings != null) {
                publicProfileSettings.guiStyle.addChangeListener(this::onStandaloneModeChanged);
            }
            this.updateStandaloneState();
        }
        this.autoHealStandaloneMode();
    }

    public void onTick() {
        this.tickStandaloneMode();
        try {
    ...
    /**
     * Per-tick update of every frame (animations, hover states, overlay panel
     * bounds, layout refresh). Runs through ClientSettingsEventForwarder each
     * tick; without it ClickGuiMainFrame.u() never executes, so overlay panels
     * (module settings, Settings page) stay at zero height and can't render or
     * receive clicks.
     */
    public void updateTickFrames() {
        try {
            ...
            for (Frame frame : this.framesToUpdate) {
                try {
                    ...
                    Umbra.debugLog((String)("" + frame.getName()));
                    Umbra.logThrowable((Throwable)exception);
                }
            }
        }

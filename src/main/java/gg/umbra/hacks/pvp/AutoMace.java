...
import gg.umbra.hacks.pvp.automace.AutoMaceRotationController;
import gg.umbra.system.AttackCancellationAdapter;
import gg.umbra.system.PhysicalAttackCancellationAdapter;
import gg.umbra.system.SharedModuleControlClaims;
import gg.umbra.system.SyntheticAttackCancellationAdapter;
...
public class AutoMace extends HackModule {
    ...
    public final OptionSetting maceSelection;
    public final OptionSetting maceType;
    public final ToggleSetting stunSlam;
    public final SliderSetting stunSlamChance;
    public final ToggleSetting aim;
    public final ToggleSetting silentAim;
    public final SliderSetting aimRange;
    public final ToggleSetting attack;
    public final RandomRangeSetting extraDelay;
    public final ToggleSetting autoUnequipElytra;
    public final ToggleSetting reEquipElytra;
    public final ToggleSetting smashOnly;
    ...
    public AutoMace() {
        ...
        this.maceType = OptionSetting.create(this, "Mace type",
        ...
        this.stunSlam = ToggleSetting.create(this, "Stun slam", false,
        ...
        this.stunSlamChance = SliderSetting.create(this, "Chance", "#", "%", 0.0, 100.0, 100.0, 1.0,
                "Chance that Stun slam will trigger");
        this.aim = ToggleSetting.create(this, "Aim", false,
        ...
        this.aimRange = SliderSetting.create(this, "Aim range", "#.#", "", 2.0, 6.0, 10.0, 0.1,
                "Maximum horizontal distance to search for targets");
        this.attack = ToggleSetting.create(this, "Attack", false, "Automatically attacks valid mace targets");
        this.extraDelay = RandomRangeSetting.createWithDescription(this, "Extra delay", "#", "ticks",
                -20.0, 0.0, 0.0, 20.0, 0.1,
                "Extra delay after attack cooldown(in ticks)\nNegative values will attack before cooldown is complete");
        this.autoUnequipElytra = ToggleSetting.create(this, "Auto unequip Elytra", false,
                "Equips a hotbar chestplate when your predicted fall can reach a mace target");
        this.reEquipElytra = ToggleSetting.create(this, "Re-equip Elytra", false,
        ...
        this.limitToItems = ToggleSetting.create(this, "Limit to items", false);
        this.allowedItems = ItemFilterList.create(this, "am-alloweditems", "Allowed Items",
                ItemFilterList.ALLOW_LIST_COLOR, Arrays.asList(new ItemLimitData("swords")));
    ...
    public void onSyntheticAttack(SyntheticAttackRequestEvent event) {
        HackModule source = event.getSource();
        if (source == this || source instanceof HitSwap || source instanceof ShieldCracker) {
            return;
        }
        this.inputTimer.reset();
        if (!event.isCanceled()) {
    ...
    private void completeStunSlamFollowup(EntityPlayerSP player) {
        if (player.isNotNull() && this.isValidMaceSlot(player, this.maceSlot)) {
            player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.maceSlot);
            AttackKeyController.releaseAttackKey();
    ...
    private boolean isValidMaceSlot(EntityPlayerSP player, int slot) {
        ...
        return this.isMace(player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().c(slot));
    }
    ...
    private void updateAutoAttack(EntityPlayerSP player) {
        ...
        if (target == null || !this.isAutoAttackReady(player, target,
                (float)(-this.extraDelay.getRandomRangeSetting()))) {
            return;
        }
        ...
        boolean stunSlamReady = this.stunSlam.getEffectiveValue().booleanValue()
                ...
                && this.findAxeSlot(player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6()) >= 0;
    ...
    private void updateAim(EntityPlayerSP player, WorldClient world) {
        if (!this.aim.getEffectiveValue().booleanValue() || world.isNull() || this.movementSnapshot == null
                ...
                || player.C$src$Lgg_umbra_wrapper_impl_ModelPlayer_$19uhx86().isFlying()
    ...
    private List<FallSample> simulateTrajectory(EntityPlayerSP player, WorldClient world,
        ...
        float fallDistance = player.M$src$F$ff28gb();
    ...
    private ImpactPrediction findImpact(List<FallSample> samples, EntityLivingBase target, double reach) {
        ...
        AxisAlignedBB bounds = target.u$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$kogbsu();
    ...
    private boolean updateElytraState(EntityPlayerSP player, WorldClient world) {
        if (!this.autoUnequipElytra.getEffectiveValue().booleanValue()) {
            this.restoreArmorSlot(player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6());
    ...
    private boolean isValidTarget(EntityPlayerSP player, EntityLivingBase target) {
        ...
        Entity vehicle = player.S$src$Lgg_umbra_wrapper_impl_Entity_$dgzs12();
    ...
    private double scoreAutomaticMace(int density, int breach, EntityLivingBase target) {
        ...
        double score = -1.0;
        if (density > 0) {
            score = this.estimateMaceDamage(player.M$src$F$ff28gb(), target, density, 0);
        }
        if (breach > 0) {
            score = Math.max(score, this.estimateMaceDamage(player.M$src$F$ff28gb(), target, 0, breach));
        }
        return score;
    }
    ...
    private double estimateMaceDamage(float fallDistance, EntityLivingBase target, int density, int breach) {
        ...
        double toughness = target.o(MonsterAttributesBridge.m$src$Lgg_umbra_wrapper_impl_Holder_$1lgjxui());
    ...
    private double getMaceReach() {
        Reach reach = Umbra.INSTANCE.getHackManager().getMod(Reach.class);

...
extends HackModule {
    ...
    public final ToggleSetting limitToItems = ToggleSetting.create(this, "Limit to items", false);
    private boolean swapActive;
    public final ToggleSetting smashOnly;
    public final ToggleSetting densityMaces;
    private int originalSlot = -1;
    public final ToggleSetting maces = ToggleSetting.create(this, "Maces", true);
    public final ToggleSetting swords;
    ...
    public void onTick(EventPreTick eventPreTick) {
        ...
        if (this.breachSwapPending && this.swapActive && this.tickCounter >= 1) {
            ...
            if (maceSlot != -1) {
                eventPreTick.getThePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(maceSlot);
                AttackKeyController.releaseAttackKey();
        ...
        if (this.swapActive) {
            ...
            if (this.tickCounter++ > restoreDelayTicks) {
                if (this.originalSlot != -1) {
                    eventPreTick.getThePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.originalSlot);
                    this.originalSlot = -1;
                }
                this.swapActive = false;
                this.tickCounter = 0;
    ...
    private void handleAttack(AttackCancellationAdapter cancellation) {
        ...
        if (this.stunSlam.getEffectiveValue().booleanValue() && this.maces.getEffectiveValue().booleanValue()
            ...
            if (heldItem.getItem().isInstance(MappedClasses.YP)) {
                this.originalSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
            ...
            if (axeSlot >= 0) {
                ...
                this.originalSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
                player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(axeSlot);
        ...
        if (weaponSlot >= 0) {
            cancellation.setCancelled(true);
            this.originalSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
            player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(weaponSlot);
            this.swapActive = true;
            this.tickCounter = 0;
            this.swapAttackPending = true;
        }
    ...
    public HotbarSwap() {
        super("HotbarSwap", -16732037, Category.UTILITY, "Swaps into another weapon on attack, copying its attributes\nAKA BreachSwap, ZeroTick");
        ...
        this.stunSlam = ToggleSetting.create(this, "Stun slam", false, "When holding an axe and attacking a shielded player:\nHits with axe first (breaks shield), then swaps to mace for a follow-up slam");
        this.axes = ToggleSetting.create(this, "Axes", true);
        this.swords = ToggleSetting.create(this, "Swords", false);
        this.allowedItems = ItemFilterList.create(this, "bs-alloweditems", "Allowed Items", ItemFilterList.ALLOW_LIST_COLOR, Arrays.asList(new ItemLimitData("swords")));

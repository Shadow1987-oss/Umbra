...
import gg.umbra.hacks.pvp.crystalaura.CrystalAuraPlacementSubModule;
import gg.umbra.hacks.pvp.crystalaura.CrystalAuraTargetSubModule;
...
import gg.umbra.wrapper.impl.Enchantments;
...
extends HackModule {
    ...
    protected float applyPotionAndEnchantmentReductions(EntityPlayerSP player, float damage) {
        ...
        float blastProtection = ObsidianCracker.getBlastProtectionLevel(player);
        if (blastProtection > 0.0f) {
            damage = ObsidianCracker.reduceDamageByBlastProtection(damage, blastProtection);
        }
        return damage;
    }
    ...
    protected float applyArmorReduction(EntityPlayerSP player, float damage) {
        return ObsidianCracker.reduceDamageByArmor(damage, (float)player.o(MonsterAttributesBridge.L()), (float)player.o(MonsterAttributesBridge.m$src$Lgg_umbra_wrapper_impl_Holder_$1lgjxui()));
    }
    ...
    public float calculateExplosionDamage(EntityPlayerSP player, ExplosionType explosionType, Vec3 explosionPosition, World world) {
        ...
        if (insideExplosionBounds) {
            ...
            if (normalizedDistance <= 1.0) {
                float blockDensity = ObsidianCracker.calculateBlockDensity(explosionPosition, player, world);

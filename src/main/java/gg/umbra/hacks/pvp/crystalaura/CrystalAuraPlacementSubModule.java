...
import gg.umbra.hacks.exploits.blockin.BlockPathPlanner;
import gg.umbra.hacks.exploits.blockin.BlockPlacementGraph;
import gg.umbra.hacks.pvp.ObsidianCracker;
import gg.umbra.system.MouseOverUpdateControlClaim;
...
import gg.umbra.utils.math.NumericMathUtil;
...
extends SubHack<ObsidianCracker> {
    ...
    public final OptionSetting targetMode;
    private final ToggleSetting predictAttackVelocity;
    private final OptionSetting optimizationMode;
    private final RotationManager rotationManager;
    private final ToggleSetting antiSuicide;
    ...
    private void clickUseItem() {
        ...
        KeyBinding useItemKey = Minecraft.gameSettings().b$src$Lgg_umbra_wrapper_impl_KeyBinding_$1yi3362();
    ...
    private void validateAction(EntityPlayerSP player, World world) {
        ...
        Vec3 eyePosition = player.I$src$Lgg_umbra_wrapper_impl_Vec3_$q14opk().addVector(0.0, player.X(), 0.0);
        ...
        if (action == CrystalAuraAction.PLACING_OBSIDIAN) {
            ...
            if (crosshairHit.getTypeOfHit().equals(RayTraceResult_type.block())) {
                ...
                Block hitBlock = crosshairHit.Z$src$Lgg_umbra_wrapper_impl_Block_$6x2c9a();
    ...
    private float computeDamageEfficiency(EntityLivingBase target, ExplosionType explosionType, Vec3 explosionPosition, EntityPlayerSP player, World world) {
        ...
        if (insideExplosionBounds) {
            ...
            if (normalizedDistance <= 1.0) {
                float blockDensity = ObsidianCracker.calculateBlockDensity(explosionPosition, predictedTarget, world);
    ...
    public HackDisplayInfo getModuleDisplayInfo() {
        ...
        String suffix = " " + moduleColor + "(ObsidianCracker)";
        return new HackDisplayInfo(countText, countColor, suffix);
    }
    ...
    private void reset() {
        ...
        if (this.previousSlot != -1) {
            EntityPlayerSP player = Minecraft.thePlayer();
            if (!player.isNull()) {
                player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.previousSlot);
            }
            this.previousSlot = -1;
        }
    ...
    public boolean isValidTarget(EntityLivingBase target, EntityPlayerSP player) {
        ...
        if (target.equals(player.S$src$Lgg_umbra_wrapper_impl_Entity_$dgzs12())) {
            return false;
        }
    ...
    private boolean wouldIntersect(EntityLivingBase target, BlockCoordinate crystalPosition, EntityPlayerSP player, World world) {
        ...
        if (target.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().intersects(crystalBox)) {
            intersects = true;
        }
        if (target.isInstance(MappedClasses.Yl)) {
            EntityPlayer targetPlayer = new EntityPlayer(target);
            ...
            if (predictedTarget.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().intersects(crystalBox)) {
                intersects = true;
            }
        }
        return intersects;
    ...
    private void aimAtAction(EntityPlayerSP player) {
        ...
        if (this.currentAction.getAction() == CrystalAuraAction.ATTACKING_CRYSTAL) {
            ...
            targetPoint = !crystals.isEmpty() && !hasPlacementPath
                    ? RotationUtil.M(eyePosition, crystals.get(0).R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl(), 0.0, 0.0, 0.0).toVec3()
    ...
    private CrystalAuraActionCandidate findActionAtYLevel(EntityPlayerSP player, World world, double searchRange,
        ...
        if (obsidianSlot != -1) {
            ItemStack obsidianStack = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().c(obsidianSlot);
            ...
            for (int x = playerBlockX - radius; x <= playerBlockX + radius; ++x) {
                for (int z = playerBlockZ - radius; z <= playerBlockZ + radius; ++z) {
                    ...
                    for (Entity crystal : crystals) {
                        Vec3 crystalPosition = crystal.I$src$Lgg_umbra_wrapper_impl_Vec3_$q14opk();
                        ...
                        Vec3 nearestCrystalPoint = RotationUtil.M(eyePosition,
                                crystal.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().contract(0.01, 0.0, 0.01),
                                0.0, 0.0, 0.0).toVec3();
                    ...
                    BlockData blockAbove = candidateBlock.R(EnumFacing.F$src$Lgg_umbra_wrapper_impl_EnumFacing_$glfxl5());
                    ...
                    try {
                        ...
                        PlacementTarget obsidianTarget = new PlacementTarget(
                                candidateBlock, EnumFacing.F$src$Lgg_umbra_wrapper_impl_EnumFacing_$glfxl5());
    ...
    public CrystalAuraPlacementSubModule(HackModule parent, String name) {
        super(parent, name);
        this.targetFilter = EntityTargetFilterValue.createForModule(this);
        this.centerScreen = ToggleSetting.create(this, "Center screen", true, "Renders crystal info on the center of your screen");
        this.showTarget = ToggleSetting.create(this, "Show target", false);
        this.targetColor = ColorPicker.createWithAlpha(this, "Target color", new Color(255, 40, 255), 50);
        this.attackColor = ColorPicker.create(this, "Attack color", new Color(169, 0, 255, 255));
        this.range = SliderSetting.createWithDescription(this, "Range", "#.#", "m", 0.0, 4.5, 6.0, "Range to check for targets");
        ...
        this.targetMode = OptionSetting.create((Object)this, "Target Mode", "How targets will be prioritized\nArmor will default to Distance for non player targets", (ModeSelection)this.distanceMode, this.distanceMode, this.yawMode, this.armorMode, this.healthMode);
        this.activationDelay = RandomRangeSetting.createWithDescription(this, "Delay", "#", "ms", 50.0, 50.0, 150.0, 500.0, 1.0, "Delay before activating");
        this.antiSuicide = ToggleSetting.create(this, "Anti suicide", true, "Prevents placing/breaking if it will result in fatal damage");
        this.maxSelfDamage = SliderSetting.create(this, "Max self damage", "#", "HP", 0.0, 19.0, 20.0, 1.0, "Maximum self damage allowed");
        this.rapidFireMode = new ModeOption("Rapid fire");
        this.predictMode = new ModeOption("Predict");
        this.noneMode = new ModeOption("None");
        this.optimizationMode = OptionSetting.create((Object)this, "Optimization", "Controls crystal optimization behavior\nNone - No crystal optimization\nRapid fire - Crystals are broken and replaced in same tick when possible\nPredict - Predicts explosion timing and pre-removes crystal for faster placement(potentially unsafe)", (ModeSelection)this.noneMode, this.rapidFireMode, this.predictMode, this.noneMode);
        this.rapidMinEfficiency = SliderSetting.create(this, "Rapid min efficiency", "#", "%", 0.0, 50.0, 100.0, 1.0, "Minimum damage efficiency (0-100%) to trigger rapid fire when placing/breaking crystals");
        this.predictAttackVelocity = ToggleSetting.create(this, "Predict attack velocity", true, "Predicts target movement when calculating damage after successfully attacking");
        this.autoObsidian = ToggleSetting.create(this, "Auto obsidian", false, "Automatically places obsidian to place crystals");
        this.minEfficiency = SliderSetting.create(this, "Min efficiency", "#", "%", 0.0, 50.0, 100.0, 1.0, "Minimum damage efficiency (0-100%) for placing and breaking crystals");
        ...
        this.friendManager = Umbra.INSTANCE.getFriendManager();
        this.delayTimer = new TimerUtil();
        this.crystalAura = (ObsidianCracker)parent;
    ...
    public void onTick(EventPreTick eventPreTick) {
        ...
        boolean activationDelayElapsed = this.delayTimer.hasTimeElapsed((long)this.activationDelay.getRandomRangeSetting());
        ...
        if (player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v() != requiredSlot) {
            if (this.previousSlot == -1) {
                this.previousSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
            }
            player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(requiredSlot);
        }
    ...
    private void validateCurrentAction(EntityPlayerSP player, World world) {
        ...
        if (this.currentAction != null && this.currentAction.getAction() == CrystalAuraAction.ATTACKING_CRYSTAL) {
            ...
            if (crystal == null || crystal.isNull() || crystal.M$src$Z$ff28xj()) {
                ...
                Vec3 nearestCrystalPoint = RotationUtil.M(eyePosition,
                        crystal.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl(), 0.0, 0.0, 0.0).toVec3();

...
import gg.umbra.wrapper.impl.CPacketUseEntity;
...
extends SubHack<ObsidianCracker> {
    ...
    private final ToggleSetting showTargetBlock;
    ...
    private final ToggleSetting placeObsidian;
    ...
    private final ToggleSetting antiSuicide = ToggleSetting.create(this, "Anti suicide", true, "Prevents breaking if it will result in fatal damage");
    ...
    private Color computeRenderColor(EntityPlayerSP player, World world) {
        ...
        Vec3 explosionPosition = this.state == CrystalAuraActionState.BREAKING_CRYSTAL && this.trackedCrystal != null && this.trackedCrystal.isNotNull() && !this.trackedCrystal.M$src$Z$ff28xj()
                ? this.trackedCrystal.I$src$Lgg_umbra_wrapper_impl_Vec3_$q14opk()
    ...
    private void handlePlacingCrystal(EntityPlayerSP player, World world) {
        ...
        if (!this.actionTimer.hasTimeElapsed((long)this.delay.getRandomRangeSetting())) {
        ...
        if (player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v() != this.crystalSlot) {
            if (this.savedSlot == -1) {
                this.savedSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
            }
            player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.crystalSlot);
        }
    ...
    private void handlePlacingObsidian(EntityPlayerSP player, World world) {
        ...
        if (this.obsidianSlot == -1 || !this.crystalAura.isObsidian(player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().c(this.obsidianSlot))) {
        ...
        if (player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v() != this.obsidianSlot) {
            if (this.savedSlot == -1) {
                this.savedSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
            }
            player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.obsidianSlot);
        }
        if (this.rotationController != null && this.rotationManager.getActiveController() == this.rotationController) {
            RayTraceResult crosshairHit = this.rotationManager.getNormalReachRayTrace();
            if (crosshairHit.getTypeOfHit().equals(RayTraceResult_type.block())) {
    ...
    private boolean acquireHoveredTarget(EntityPlayerSP player, WorldClient world, RayTraceResult crosshairHit) {
        ...
        if (this.isValidBaseBlock(world, hoveredPosition)) {
            this.crystalSlot = player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v();
    ...
    private Vec3 computeCrystalHitVec(EntityPlayerSP player, World world, Vec3 eyePosition, BlockData baseBlock, float currentYaw, float currentPitch) {
        EnumFacing[] facings = new EnumFacing[]{EnumFacing.F$src$Lgg_umbra_wrapper_impl_EnumFacing_$glfxl5(), EnumFacing.w(), EnumFacing.M(), EnumFacing.X(), EnumFacing.g$src$Lgg_umbra_wrapper_impl_EnumFacing_$1ii8mzu(), EnumFacing.B()};
    ...
    private void releaseRotation() {
        ...
        if (this.savedSlot != -1) {
            ...
            if (!player.isNull()) {
                player.V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().g(this.savedSlot);
            }
            this.savedSlot = -1;
        }
    }
    ...
    public void onPostTick(EventPostTick eventPostTick) {
        ...
        if (this.state == CrystalAuraActionState.BREAKING_CRYSTAL && this.trackedCrystal != null && this.trackedCrystal.isNotNull() && !this.trackedCrystal.M$src$Z$ff28xj()) {
            Vec3 crystalHitPoint = RotationUtil.M(eyePosition, this.trackedCrystal.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl(), 0.0, 0.0, 0.0).toVec3();
    ...
    private void handleBreakingCrystal(EntityPlayerSP player, World world) {
        ...
        Vec3 crystalPosition = this.trackedCrystal.I$src$Lgg_umbra_wrapper_impl_Vec3_$q14opk();
    ...
    public CrystalAuraTargetSubModule(HackModule parent, String name) {
        super(parent, name);
        this.maxSelfDamage = SliderSetting.create(this, "Max self damage", "#", "HP", 0.0, 19.0, 20.0, 1.0, "Maximum self damage allowed");
        this.delay = RandomRangeSetting.createWithDescription(this, "Delay", "#", "ms", 0.0, 0.0, 100.0, 500.0, 1.0, "Delay between break/place cycles");
        ...
        this.placeObsidian = ToggleSetting.create(this, "Place obsidian", false, "Automatically places obsidian from hotbar when hovering a valid placement surface");
        this.showTargetBlock = ToggleSetting.create(this, "Show target block", true, "Renders a highlight on the target obsidian block");
    ...
    private boolean isHoldingCrystal(EntityPlayerSP player) {
        return player.isNotNull() && this.crystalAura.isEndCrystal(player.B$src$Lgg_umbra_wrapper_impl_ItemStack_$impdvt());
    }

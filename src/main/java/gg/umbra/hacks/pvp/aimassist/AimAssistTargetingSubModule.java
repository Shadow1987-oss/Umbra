...
extends SubHack<AutoAim> {
    ...
    private double[] resolveSnapPoint(EntityPlayerSP player, EntityLivingBase target) {
        AutoAim aimAssist = (AutoAim)this.getParent();
        double playerEyeY = player.A() + (double)player.X();
        double targetMinY = target.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().getMinY();
        double targetMaxY = target.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().getMaxY();
    ...
    private void runTargetLoop() {
        ...
        if (aimAssist.getRequireMouseDown().getEffectiveValue().booleanValue() && !gg.umbra.config.ClientSettings.isAttackButtonDown()) {
            this.target = null;
            this.reset();
            return;
        }
        ...
        if (aimAssist.getRequireMouseDown().getEffectiveValue().booleanValue() && gg.umbra.config.ClientSettings.isAttackButtonDown() && this.target == null || !aimAssist.getRequireMouseDown().getEffectiveValue().booleanValue()) {
    ...
    public AimAssistTargetingSubModule(HackModule parent, String name) {
        super(parent, name);
    }

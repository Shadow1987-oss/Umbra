...
import gg.umbra.utils.cache.CachedBoolean;
import gg.umbra.utils.cache.CachedFloat;
...
public class RenderEntityContext {
    ...
    public boolean isAttackable() {
        if (!this.attackableCache.isCached()) {
            if (this.entityPlayer != null) {
                this.attackableCache.setValue(Umbra.INSTANCE.getClientSettings().isTeammate(this.viewer, this.entity));
            } else {
    ...
    public String getNameTag() {
        if (this.cachedNameTag == null && this.entityPlayer != null) {
            this.cachedNameTag = Umbra.INSTANCE.getHackManager().getMod(EntityLabels.class).Q(this.viewer, this, this.entityPlayer);
        }
    ...
    public ItemStack getHeldItem() {
        if (this.heldItem == null) {
            ItemStack itemStack = this.entity.B$src$Lgg_umbra_wrapper_impl_ItemStack_$impdvt();
    ...
    public MutableColor getRenderColor(boolean outline) {
        if (outline) {
            if (this.outlineColor == null) {
                this.outlineColor = Umbra.INSTANCE.getClientSettings().resolveTeamColor(this, true, true);
            }
            return this.outlineColor;
        }
        if (this.fillColor == null) {
            this.fillColor = Umbra.INSTANCE.getClientSettings().resolveTeamColor(this, false);
        }
    ...
    public boolean isFriend() {
        if (!this.friendCache.isCached()) {
            if (this.entityPlayer != null) {
                this.friendCache.setValue(Umbra.INSTANCE.getFriendManager().isFriend(this.getName()));
            } else {
    ...
    public boolean isEnemy() {
        if (!this.enemyCache.isCached()) {
            if (this.entityPlayer != null) {
                this.enemyCache.setValue(Umbra.INSTANCE.getEnemyManager().isEnemy(this.getName()));
            } else {
    ...
    public ModelPlayer getModelPlayer() {
        if (this.modelPlayer == null && this.entityPlayer != null) {
            this.modelPlayer = this.entityPlayer.C$src$Lgg_umbra_wrapper_impl_ModelPlayer_$19uhx86();
        }
    ...
    public boolean isBot() {
        if (!this.botCache.isCached()) {
            this.botCache.setValue(Umbra.INSTANCE.getClientSettings().isBot(this.entity));
        }

...
import gg.umbra.wrapper.impl.PlayerDiggingAction;
...
extends AnimationsMode {
    ...
    public void onTick(EventPreTick event) {
        ...
        if (this.isHoldingSword() && ((Animations)this.getParent()).requiresMouseDown() && ((Animations)this.getParent()).ignoreManualBlock.getEffectiveValue().booleanValue() && !this.bufferingPackets && gg.umbra.config.ClientSettings.isUseItemButtonDown() && Minecraft.thePlayer().o$src$Z$1iprrmi()) {
    ...
    public void onPacketSend(EventPacketSend event) {
        ...
        if (this.isReleaseUseItemPacket(packet)) {
            ...
            this.delayMillis = (int)this.delay.getRandomRangeSetting();
    ...
    public LegacyBlockingPacketBufferedAnimationsMode(HackModule parent, String name) {
        super(parent, name);
        this.queuedPackets = new LinkedList<EventPacketSend>();
        this.delay = RandomRangeSetting.create(this, "Delay", "#", "", 0.0, 50.0, 100.0, 500.0);

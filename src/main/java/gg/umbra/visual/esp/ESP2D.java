...
import gg.umbra.visual.entity.ProjectedEntityBounds;
...
import gg.umbra.wrapper.impl.RenderWorldLastEvent;
...
extends SubHack<ESP> {
    ...
    public void onRender2D(EventRender2D event) {
        ...
        SmoothFontRenderer fontRenderer = Umbra.INSTANCE.getFontManager().W(0.9, true);
        ...
        for (ProjectedEntityBounds projectedEntityBounds : this.pendingBounds) {
            ...
            if (projectedEntityBounds.entity.isInstance(MappedClasses.zm)) {
                ...
                if (this.parentEsp.showName.getEffectiveValue().booleanValue()) {
                    ...
                    if (friend && (friendEntry = Umbra.INSTANCE.getFriendManager().findTargetedFriend(projectedEntityBounds.context.getName())) != null) {
    ...
    public void onPreRenderLiving(EventPreRenderLiving event) {
        ...
        AxisAlignedBB worldBounds = entity.R$src$Lgg_umbra_wrapper_impl_AxisAlignedBB_$r19dfl().expand(expansion, expansion, expansion);
    ...
    public ESP2D(HackModule parent, String name) {
        super(parent, name);

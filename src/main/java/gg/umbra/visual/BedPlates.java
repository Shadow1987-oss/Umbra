...
import gg.umbra.visual.bedplates.BedPlateBlockStateKey;
import gg.umbra.visual.bedplates.BedPlateCountState;
import gg.umbra.visual.hud.FreeLookHudModule;
import gg.umbra.worldmods.bedbreaker.BedTargetRenderPosition;
import gg.umbra.ui.font.BufferedSmoothFontRenderer;
...
import gg.umbra.wrapper.impl.ClientChunkProvider;
...
import gg.umbra.wrapper.impl.Timer;
...
extends HackModule {
    ...
    private final ToggleSetting showDistance = ToggleSetting.create(this, "Show distance", true);
    ...
    private void renderPlate(EventRender3D event, BedPlateCountState countState) {
        ...
        if (ForgeVersion.MC_1_16_5.d()) {
            if (Minecraft.gameSettings().x() == 0) {
                ...
                ActiveRenderInfo renderInfo = Minecraft.m$src$Lgg_umbra_wrapper_impl_EntityRenderer_$13begmf().l();
        ...
        SmoothFontRenderer fontRenderer = Umbra.INSTANCE.getFontManager().W(1.2, false);
                    ...
                    for (int offsetX = minX; offsetX <= maxX; ++offsetX) {
                        for (int offsetZ = minZ; offsetZ <= maxZ; ++offsetZ) {
                            ...
                            if (ForgeVersion.MC_1_16_5.d()) {
                                ...
                                BlockState blockState = new BlockState(Umbra.INSTANCE.getMappings().Cy.u(world.getObject(), blockPos.getObject()));

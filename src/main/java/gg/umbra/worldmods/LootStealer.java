...
import gg.umbra.worldmods.cheststeal.ChestStealBestSlotTracker;
import gg.umbra.worldmods.cheststeal.ChestStealSlotDistanceComparator;
...
import gg.umbra.wrapper.impl.Inventory;
...
implements InventoryActionModule {
    ...
    private final ToggleSetting shuffle;
    private final TimerUtil screenReadyTimer;
    private final ItemFilterList blacklist;
    private final ToggleSetting keepOpen;
    private final TimerUtil actionDelayTimer;
    private final int[] helmetItemIds;
    private int previousSlot = -1;
    private final RandomRangeSetting clickDelay;
    private final ArrayList<Integer> pendingSlots;
    private boolean queueInitialized;
    private boolean waitingToClose;
    private final ToggleSetting checkMenu = ToggleSetting.create(this, "Check in menu", false, "This will attempt to ignore Menus such as\nServer selectors, and settings inventory menus\nThis may not work 100% on all servers");
    private final ToggleSetting bestOnly = ToggleSetting.create(this, "Best only", false, "Only takes an item if it is better than what you have equipped.");
    ...
    private void scanPlayerInventory() {
        ...
        for (int i = 0; i < 45; ++i) {
            ItemStack itemStack = entityPlayerSP.F$src$Lgg_umbra_wrapper_impl_Container_$152y6lm().getSlot(i).getStack();
    ...
    public LootStealer() {
        ...
        this.shuffle = ToggleSetting.create(this, "Shuffle", false, "Take items in a random order");
        this.clickDelay = RandomRangeSetting.createWithIncrement(this, "Click delay", "#", "", 50.0, 150.0, 200.0, 300.0, 5.0);
        this.blacklist = ItemFilterList.create(this, "cheatsteal-blacklisted", "Blacklisted", ItemFilterList.BLOCK_LIST_COLOR, Collections.emptyList());
    ...
    public void onTick(EventPrePlayerTick eventPrePlayerTick) {
        if (Umbra.INSTANCE.getHackManager().isOtherInventoryActionActive(LootStealer.class) || Umbra.INSTANCE.getClientSettings().isLobbyCheckActive()) {
            this.stealing = false;
            return;
        }

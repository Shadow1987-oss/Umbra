...
implements ToggleableListEntry {
    ...
    public boolean matches(ItemStack itemStack) {
        try {
            ...
            if (this.getName().toLowerCase().startsWith("slot")) {
                ...
                if (slotNumber >= 1 && Minecraft.thePlayer().V$src$Lgg_umbra_wrapper_impl_InventoryPlayer_$erqak6().v() + 1 == slotNumber) {
                    return true;
                }
            }
    ...
    private boolean matchesRegisteredItem(ItemStack itemStack) {
        return Umbra.INSTANCE.getItemHelper().matchesItem(this.getName().toLowerCase(), itemStack);
    }

...
import gg.umbra.mapping.expr.MethodCallInjectionExprEditor;
import gg.umbra.mapping.expr.MethodCallLineCaptureExprEditor;
import gg.umbra.mapping.expr.MethodCallReplacementExprEditor;
import gg.umbra.runtime.ClassBytecodeCache;
import gg.umbra.ui.click.component.GuiComponent;
import gg.umbra.wrapper.impl.LaunchClassLoader;
...
implements MappingTask {
    ...
    public String z(Class<?> clazz, String string, String string2, String string3, String string4) {
        ...
        string5 = string5 + "if(" + string + "." + EventDispatcher.getFireMethod(clazz).getName() + "()) { return " + string3 + "; } " + string4;
        ...
        if (n != 0) {
            ...
            if (bl4) {
                ...
                Method method = EventDispatcher.findEventListenersAccessor(eventClass);
                string5 = "if(" + clazz.getName() + "#" + method.getName() + "()." + EventDispatcher.getHasListenersMethod().getName() + "()) { " + string5 + "}";
            }
    ...
    private void dumpFailedCommit(int errorCode) {
        try {
            File directory = new File(System.getProperty("java.io.tmpdir"), "UmbraCache");
            if (!directory.exists()) {
                directory.mkdirs();
            }
            ...
            Umbra.debugLog("Failed mapping commit bytecode dumped to " + directory.getAbsolutePath());
        }
        catch (Exception exception) {
            Umbra.logThrowable(exception);
        }
    ...
    public CtBehavior F(MappingMethod mappingMethod) {
        try {
            ...
            Umbra.logThrowable(notFoundException);
            return null;
        }
    }

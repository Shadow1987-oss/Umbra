...
import gg.umbra.mapping.InsertedCallbackEventMarker;
...
public class EventInjectionSpec {
    ...
    public String buildInjectionCode() {
        ...
        if (this.useCancelableReturnGuard) {
            ...
            eventStatement = "boolean " + returnGuardName + " = " + generatedEventName + "." + EventDispatcher.getFireMethod(this.eventClass).getName() + "();";
            eventStatement = (String)eventStatement + "if(" + returnGuardName + ") { return " + this.returnExpression.replace("$event", generatedEventName) + "; } ";
        } else {
            eventStatement = "new " + this.eventClass.getName() + "(" + (this.constructorArguments == null ? "" : this.constructorArguments) + ")." + EventDispatcher.getFireMethod(this.eventClass).getName() + "();";
        }
        ...
        if (hasInsertedCallbackMarker) {
            listenerAccessor = EventDispatcher.findEventListenersAccessor(this.eventClass);
            injectionStatement = "if(" + this.eventClass.getName() + "#" + ((Method)listenerAccessor).getName() + "()." + EventDispatcher.getHasListenersMethod().getName() + "()) { " + injectionStatement + "}";
        }

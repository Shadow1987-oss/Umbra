...
public class RuntimeNameMappingRegistry {
    ...
    public static void initializeRegistry() {
        ...
        switch (forgeVersion) {
            ...
            default: {
                if (memberNameRemapTable == null) {
                    Umbra.logTimed("No member remap table for version " + forgeVersion + ", using V110 fallback");
                    memberNameRemapTable = new MemberNameRemapTableV110();
                }
            }
        }
    ...
    static {
        ...
        switch (forgeVersion) {
            ...
            default: {
                Umbra.logTimed("Unknown protocol version " + forgeVersion + ", falling back to latest mappings (V110)");
                classNameRemapTable = new ClassNameRemapTableV110();
            }
        }
        if (ForgeVersion.MC_1_16_5_ACTUAL.Y() && !Umbra.INSTANCE.isForgeAbsent()) {
    ...
    public static String remapClassName(String sourceClassName) {
        ...
        String remapped = classNameRemapTable.lookupRemappedClassName(sourceClassName);
        return remapped != null ? remapped : sourceClassName;
    }
}

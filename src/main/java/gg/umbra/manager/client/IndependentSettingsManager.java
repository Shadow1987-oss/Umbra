...
public class IndependentSettingsManager {
    ...
    public JsonArray toJson() {
        ...
        enemies.add("enemies", (JsonElement)Umbra.INSTANCE.getEnemyManager().toJson());
        ...
        inventoryManager.add("inventoryManager", (JsonElement)Umbra.INSTANCE.getInventoryFilterPresetRegistry().toJson());
    ...
    public void loadIndependentSettings(JsonArray serializedSettings) {
        ...
        for (int index = 0; index < serializedSettings.size(); ++index) {
            ...
            if (setting.get("enemies") != null && !setting.get("enemies").isJsonNull()) {
                ...
                Umbra.INSTANCE.getEnemyManager().loadJson(nestedArray);
            }
            if (setting.get("frames") != null && !setting.get("frames").isJsonNull()) {
                nestedArray = setting.get("frames").getAsJsonArray();
                if (!Umbra.INSTANCE.getPublicProfileSettings().framePositionsPerProfile.getEffectiveValue().booleanValue()) {
            ...
            Umbra.INSTANCE.getInventoryFilterPresetRegistry().loadJson(inventoryManager);
        }
    }

...
import gg.umbra.value.DirectValueAccessor;
import gg.umbra.value.EntityTargetFilterValue;
import gg.umbra.value.ListValue;
import gg.umbra.value.ValueAccessor;
import gg.umbra.value.ValueChangeListener;
import gg.umbra.value.ValueChangeValidator;
...
implements INamed {
    ...
    public Value(Object owner, String name, K defaultValue) {
        ...
        // uninitialized. SliderSetting overrides A(), so virtual dispatch here
        // left its default value null and also ran change side effects.
        this.defaultValue = defaultValue;
        this.currentValue = defaultValue;
        if (owner != null) {
            if (owner instanceof ColorPicker || owner instanceof EntityTargetFilterValue) {
                return;
            }
            if (owner instanceof PublicProfileSettings) {
                Umbra.INSTANCE.getSettingsManager().registerValue(this);
            } else if (!(owner instanceof HackModule)) {
                Umbra.INSTANCE.getValueManager().registerValue(this);
            }
        }
    }
    ...
    public String getId() {
        ...
        if (this.owner != null && this.owner instanceof SubHack) {
            SubHack subModule = (SubHack)this.owner;

...
extends Value<Boolean[], EntityTargetFilterValue> {
    private final ToggleSetting playersValue = ToggleSetting.createWithDisplayName(this, "Players" + this.getId(), "Players", true);
    private final ToggleSetting ignoreBehindWallsValue;
    private final List<ToggleSetting> filterValues;
    private final ToggleSetting ignoreNakedValue;
    private final ToggleSetting ignoreInvisibleValue;
    private final ToggleSetting peacefulValue;
    boolean synchronizingValues;
    private final ToggleSetting neutralValue;
    private final ToggleSetting mobsValue = ToggleSetting.createWithDisplayName(this, "Mobs" + this.getId(), "Mobs", false);
    ...
    public ToggleSetting getNeutralValue() {
        return this.neutralValue;
    }

    public List<ToggleSetting> getFilterValues() {
        return this.filterValues;
    }

    public EntityTargetFilterValue(Object owner, String name) {
        super(owner, name, new Boolean[7]);
        this.peacefulValue = ToggleSetting.createWithDisplayName(this, "Peaceful" + this.getId(), "Peaceful", false);
        this.neutralValue = ToggleSetting.createWithDisplayName(this, "Neutral" + this.getId(), "Neutral", false);
        this.ignoreNakedValue = ToggleSetting.createWithDisplayName(this, "Ignore Naked" + this.getId(), "Ignore naked", false);
        this.ignoreInvisibleValue = ToggleSetting.createWithDisplayName(this, "Ignore invisible" + this.getId(), "Ignore invisible", false);
        this.ignoreBehindWallsValue = ToggleSetting.createWithDisplayName(this, "Ignore behind walls" + this.getId(), "Ignore behind walls", false);
    ...
    public ToggleSetting getIgnoreBehindWallsValue() {
        return this.ignoreBehindWallsValue;
    }

    public ToggleSetting getPeacefulValue() {
        return this.peacefulValue;
    }

    public String toString() {
        StringBuilder serializedFilters = new StringBuilder();
        for (ToggleSetting filterValue : this.filterValues) {
    ...
    public ToggleSetting getIgnoreNakedValue() {
    ...
    public ToggleSetting getMobsValue() {
    ...
    private void handleFilterValueChanged(ToggleSetting changedFilterValue) {
    ...
    public ToggleSetting getIgnoreInvisibleValue() {
    ...
    public ToggleSetting getPlayersValue() {
        return this.playersValue;
    }

    public static EntityTargetFilterValue createForModule(HackModule module) {
    ...
    public boolean isValidTarget(Entity entity) {
        ...
        if (Umbra.INSTANCE.getFriendManager().isFriend(entityLivingBase)) {
            return false;
        }
        ...
        if (isPlayer) {
            ...
            if (Umbra.INSTANCE.getEnemyManager().isEnemy(entity.getName())) {
                return true;
            }
            if (this.ignoreNakedValue.getEffectiveValue().booleanValue() && RotationUtil.b$src$Z$reqs95(entityLivingBase)) {
                return false;
            }
            if (Umbra.INSTANCE.getClientSettings().isTeammate(entityPlayerSP, entityLivingBase)) {
                return false;
            }
            return !Umbra.INSTANCE.getClientSettings().isBot(entityLivingBase);
        }

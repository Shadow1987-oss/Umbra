...
import gg.umbra.feature.Module;
import gg.umbra.input.BindActivationMode;
...
import gg.umbra.value.ModuleValueDisplayFormatter;
...
EventListener {
    ...
    private final List<SubHack> subModules;
    ...
    public void registerSubModule(SubHack ... subModuleArray) {
    ...
    public SubHack getSubModule(String string) {
        for (SubHack subModule : this.getSubModules()) {
    ...
    public void loadJson(JsonObject jsonObject) {
        ...
        String id = ConfigJsonUtils.getString(jsonObject, "id");
        String name = ConfigJsonUtils.getString(jsonObject, "name");
        // Id-keyed configs survive display renames; legacy blocks without id fall back to name.
        if (id != null ? id.equalsIgnoreCase(this.getId()) : (name != null && name.equalsIgnoreCase(this.getName()))) {
            Object object;
            JsonArray jsonArray;
            if (this.bind.usesOwnKeybindStorage()) {
    ...
    public void showToggleNotification() {
        if (Umbra.INSTANCE.getFontManager() == null || Umbra.INSTANCE.getNotificationManager() == null) {
            return;
        }
        ...
        Umbra.INSTANCE.getNotificationManager().show(this.toggleNotification);
    }
    ...
    public HackModule(String string, int n) {
    ...
    public void setEnabled(boolean enabled, boolean bypassVisibilityCheck) {
        ...
        if (!bypassVisibilityCheck && !this.isVisible() && this.category != Category.NONE && enabled) {
            if (Umbra.INSTANCE.getNotificationManager() != null) {
                Umbra.INSTANCE.getNotificationManager().show("Hidden Module", "Attempted to toggle " + this.getName() + "!", NotificationType.WARNING, 2500L);
            }
            return;
        }
        if (enabled && this.isBlatantMod()) {
            Umbra.INSTANCE.getPrimaryMappingTaskSet().f();
        }
        if (!enabled) {
        ...
        if (this.enabled) {
            EventDispatcher.getInstance().registerListener(this, this.w());
        ...
        if (stateChanged) {
            ...
            if (this.category != Category.NONE || this instanceof HudModule) {
    ...
    public void p(SubHackValue subModuleValue, SubHackValue subModuleValue2) {
        if (this instanceof SubHack) {
            return;
        }
        if (!this.enabled) {
            return;
        ...
        if (((SubHack)t).isParentEnabled()) {
            ((HackModule)t).setEnabled(false);
        }
        if (!((SubHack)t2).isParentEnabled()) {
            ((HackModule)t2).setEnabled(true);
        }
    }

    public boolean q$src$Z$12h8h4c() {
        return this.getCategory() != Category.NONE && !(this instanceof UtilityHack);
    }

    public void K(boolean bl) {
        if (bl) {
            Umbra.INSTANCE.getModuleProfileMetadataCodec().addModule(this);
        } else {
            Umbra.INSTANCE.getModuleProfileMetadataCodec().removeModule(this);
        }
    }

    public HackModule(String string, int n, Category category) {
    ...
    public HackModule(String string, int n, Category category, String string2) {
    ...
    public List<SubHack> getSubModules() {
    ...
    public void j() {
        if (!this.developmentWarningShown && this.Q()) {
            ...
            Umbra.INSTANCE.getNotificationManager().show("Module in development", this.getName() + " is in development\n\nUse with caution and report issues to support", NotificationType.WARNING, 10000L);
        }
    }
    ...
    public HackModule(String string) {
        this(string, 0, Category.NONE);
    }

    public HackModule(String string, int n, int n2, Category category, String string2) {
        this.values = new ArrayList();
        this.subModules = new ArrayList<SubHack>();
        ...
        this.bind = this.C$src$Lgg_umbra_unmap_Bendable_$1we4j6l();
